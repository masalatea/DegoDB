...
<title><?php print getres("TITLE_SPEC_EDIT"); ?> - <?php print getres("TITLE_TOP"); ?></title>
// End Template Content
...
$HeaderCaption = "Select Project to add Spec";
if ($ProjectPID != "") {
	
	$DAProject = new ProjectDBAccess();
	$thisProjectObj = $DAProject->GetProject($ProjectPID);
	if ($thisProjectObj) {
		$HeaderCaption = "Confirm Project to add Spec: " . $thisProjectObj->name;
	} else {
		die("Fatal Error. Unknown Project");
	}
}
printPathOnTopForSpec($HeaderCaption, $ProjectPID, "", "");

$DAProject = new ProjectDBAccess();
$projectlist = $DAProject->GetProjectbyOwnerOrUserSecurityList($matsuesoft_login_token_id); 
if (count($projectlist) > 0) {
	?>
    <form action="spec_edit.php" method="post" id="speceditform">
    <?php
	if ($ProjectPID != "") {
		$DAProject = new ProjectDBAccess();
		$thisProjectObj = $DAProject->GetProject($ProjectPID);
