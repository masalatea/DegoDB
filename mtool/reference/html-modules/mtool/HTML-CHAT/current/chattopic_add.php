...
<title><?php print getres("TITLE_CHAT_TOPIC_EDIT"); ?> - <?php print getres("TITLE_TOP"); ?></title>
// End Template Content
...
$HeaderCaption = "Select Project to add Chat Topic";
if ($ProjectPID != "") {
	$HeaderCaption = "Confirm Project to add Chat Topic";
}
printPathOnTopForChat($HeaderCaption, $ProjectPID);

$DAProject = new ProjectDBAccess();
$projectlist = $DAProject->GetProjectbyOwnerOrUserSecurityList($matsuesoft_login_token_id); 
if (count($projectlist) > 0) {
	?>
    <form action="chattopic_edit.php" method="post" id="chattopiceditform">
    <?php
	if ($ProjectPID != "") {
		$DAProject = new ProjectDBAccess();
