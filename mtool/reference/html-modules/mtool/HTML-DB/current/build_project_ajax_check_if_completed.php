		...
		if (CheckIfLoginUserIsUserAndOutputMessage($ProjectPID, $matsuesoft_login_token_id)) {
			...
			if (is_numeric($ProjectPID)) {
				if ($BuildTokenString != "") {
					...
					$BuildToken = $DABuildToken->GetBuildToken($BuildTokenString, $ProjectPID);
					
					if ($BuildToken) {
						$DABuildTokenProjectSourceOutput = new BuildTokenProjectSourceOutputDBAccess();
						$BuildTokenProjectSourceOutputList = $DABuildTokenProjectSourceOutput->GetBuildTokenProjectSourceOutputList($BuildToken->PID, $ProjectPID);
						
						if ($BuildTokenProjectSourceOutputList && is_array($BuildTokenProjectSourceOutputList) && count($BuildTokenProjectSourceOutputList) > 0) {
