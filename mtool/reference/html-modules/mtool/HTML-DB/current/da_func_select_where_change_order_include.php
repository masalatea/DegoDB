...
if (!$DBWritePermission) {
	?>
    <H3><font color="red">ERROR! No Permission to Access</font></H3>
    <?php
	$NoError = false;
}
...
if ($NoError) {
	...
	printPathOnTopForDBAccessClass("Change Order of Select's Where", $ProjectPID, $DAPID, $DAFuncPID, "", "", "", "", "");
	...
	if ($NewSortOrder != "") {
		...
		if (count($NewSortOrder) >0) {
			for ($i = 0 ; $i < count($NewSortOrderList) ;$i++) {
				...
				if($DAdafuncselectwhere->UpdatedafuncselectwhereOrder($i + 1, $thisPID, $ProjectPID) === FALSE) {
					// Failed
					?>
	...
	if ($doReset != "") {
		...
		for($i = 0 ; $i < count($dafuncselectwherelist); $i++) {
			...
			if($DAdafuncselectwhere->UpdatedafuncselectwhereOrder("Default(WhereOrder)", $dafuncselectwhere->PID, $ProjectPID) === FALSE) {
				// Failed
				?>
				<h3><font color="red">Error! Failed to update. something strange. Please ask administrator if this continues.</font></h3>
				<?php
