...
if ($NoError) {
	...
	if ($insertToken != "") {
		...
		$__OBJECT_NAME__->PID = GetPrimaryKeyValueIfAlreadyInsertedForThisHost($insertToken);
	}
	
	if ($__OBJECT_NAME__->PID == "") {
		// Add
		if ($UPDATE != "") {
			...
			if($insertResult === FALSE) {
				...
				// Success
				$__OBJECT_NAME__->PID = $__DB_OBJECT_NAME__->insert_id;
				// == START OF EDITABLE AREA FOR "Insert Data - Success" ==
				?>
                <h3><font color="red">Inserted</font></h3>
                <?php
				// == END OF EDITABLE AREA FOR "Insert Data - Success" ==
			}
			
			// Please set $__OBJECT_NAME__->PID in above Editable Area to perform this.
			if ($insertToken != "" && is_numeric($__OBJECT_NAME__->PID)) {
				if (SetPrimaryKeyValueForInsertTokenForThisHost($insertToken, $__OBJECT_NAME__->PID)) {
					// Success
					$insertToken = "";
				} else {
					// Failed
	...
	} else if (is_numeric($__OBJECT_NAME__->PID)) {
		// Select/Update
		$needToLoad = true;
		...
		if ($DELETE != "") {
			...
			if($deleteResult === FALSE) {
				...
				?>
                <h3><font color="red">Deleted</font></h3>
                <?php
	...
	if ($showForm && $__OBJECT_NAME__ != NULL) {
