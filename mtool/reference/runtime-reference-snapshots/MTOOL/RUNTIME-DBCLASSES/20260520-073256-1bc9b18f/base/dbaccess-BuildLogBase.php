<?php

// AUTO-GENERATED BASE FILE.
// Do not edit this file manually or with AI/Codex.
// Keep customizations in the wrapper/custom class and use this base file exactly as regenerated by the tool.

require_once __DIR__ . '/../_support/legacy-dbaccess/dbaccess-BuildLog.php';

// Generated from canonical DB Access metadata.
// Simple CRUD / joined select methods regenerate SQL when canonical metadata is sufficient.
// Known helper-style methods regenerate canonical PHP bodies when SQL regeneration is not the right fit.
// Remaining unsupported methods continue to delegate to copied legacy support under `_support/legacy-dbaccess/`.

class BuildLogDBAccessBase extends BuildLogDBAccessLegacy
{
    // source_of_truth=sync-bootstrap action_type=UNKNOWN order=11 generation=canonical-constructor
    // reason=bootstrap constructor is empty, so runtime owns the no-op constructor directly
    public function __construct()
    {
    }

    // source_of_truth=sync-bootstrap action_type=SELECTLIST order=14 generation=canonical-sql
    public function GetBuildLogList($param_BuildLog_BuildTokenPID_where)
    {
        global $mtooldb, $last_sql_command_for_mtooldb;
        connect_mtooldb_if_not_yet();
        reconnect_mtooldb_if_necessary();

        $result = array();

        $last_sql_command_for_mtooldb = 'select BuildLog.PID, BuildLog.BuildTokenPID, BuildLog.MessageType, BuildLog.Message, BuildLog.AddedDT from BuildLog' . ' where ' . 'BuildLog.BuildTokenPID = ' . '\'' . $mtooldb->real_escape_string($param_BuildLog_BuildTokenPID_where) . '\'';
        $ret = $mtooldb->query($last_sql_command_for_mtooldb);
        if ($mtooldb->errno != 0) {
            error_log("Error occured while executing SQL: " . $mtooldb->error . " in " . __FILE__ . " on line " . __LINE__);
            return $ret;
        }
        while($thisline=$ret->fetch_row()) {
            $thisresult = new BuildLogData();
            $thisresult->PID = $thisline[0];
            $thisresult->BuildTokenPID = $thisline[1];
            $thisresult->MessageType = $thisline[2];
            $thisresult->Message = $thisline[3];
            $thisresult->AddedDT = $thisline[4];
            array_push($result, $thisresult);
        }
        return $result;
    }

    // source_of_truth=sync-bootstrap action_type=INSERT order=42 generation=canonical-sql
    public function InsertBuildLog($BuildLogObj)
    {
        global $mtooldb, $last_sql_command_for_mtooldb;
        connect_mtooldb_if_not_yet();
        reconnect_mtooldb_if_necessary();

        $last_sql_command_for_mtooldb = 'insert into BuildLog (BuildTokenPID, MessageType, Message) values(' . '\'' . $mtooldb->real_escape_string($BuildLogObj->BuildTokenPID) . '\'' . ', ' . '\'' . $mtooldb->real_escape_string($BuildLogObj->MessageType) . '\'' . ', ' . '\'' . $mtooldb->real_escape_string($BuildLogObj->Message) . '\'' . ')';
        $result = $mtooldb->query($last_sql_command_for_mtooldb);
        if ($mtooldb->errno != 0) {
            error_log("Error occured while executing SQL: " . $mtooldb->error . " in " . __FILE__ . " on line " . __LINE__);
        }
        return $result;
    }

    // source_of_truth=sync-bootstrap action_type=DELETE order=58 generation=canonical-sql
    public function DeleteBuildLog()
    {
        global $mtooldb, $last_sql_command_for_mtooldb;
        connect_mtooldb_if_not_yet();
        reconnect_mtooldb_if_necessary();

        $last_sql_command_for_mtooldb = 'delete from BuildLog where ' . 'BuildLog.BuildTokenPID not in ' . '(select PID from BuildToken)';
        $result = $mtooldb->query($last_sql_command_for_mtooldb);
        if ($mtooldb->errno != 0) {
            error_log("Error occured while executing SQL: " . $mtooldb->error . " in " . __FILE__ . " on line " . __LINE__);
        }
        return $result;
    }
}

?>