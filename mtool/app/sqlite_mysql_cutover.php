...
const APP_SQLITE_MYSQL_CUTOVER_PLAN_VERSION = 'sqlite-mysql-cutover-plan-v1';
const APP_SQLITE_MYSQL_CUTOVER_REQUIRED_APPROVALS = ['freeze_confirmed', 'switch_approved', 'rollback_acknowledged'];
const APP_SQLITE_MYSQL_CUTOVER_OPERATOR_PACKAGE_VERSION = 'sqlite-mysql-cutover-operator-package-v1';
const APP_SQLITE_MYSQL_CUTOVER_OPERATOR_REQUIRED_APPROVALS = ['operator_package_approved', 'rollback_rehearsal_acknowledged'];

/** @param array<string,mixed> $manifest @param array<string,mixed> $verification @param array<string,mixed> $cutover @param array<string,mixed> $rollback @return array<string,mixed> */
function app_sqlite_mysql_cutover_plan(array $manifest, array $verification, array $cutover, array $rollback, array $approvals = []): array
{
    $errors = [];
    ...
    foreach (APP_SQLITE_MYSQL_CUTOVER_REQUIRED_APPROVALS as $approval) {
        if (!isset($approvalSet[$approval])) $errors[] = 'missing_approval:' . $approval;
    }

    $errors = array_values(array_unique($errors));
    $safe = [
        'cutover_plan_version' => APP_SQLITE_MYSQL_CUTOVER_PLAN_VERSION,
        'cutover_allowed' => $errors === [],
        'stage' => $errors === [] ? 'cutover_contract_ready' : 'cutover_contract_blocked',
        'mutation_performed' => false,
        'promotion_manifest_sha256' => $manifestDigest,
        'verification_sha256' => app_sqlite_mysql_promotion_digest($verification),
        'requires_explicit_approval' => true,
        'required_approvals' => APP_SQLITE_MYSQL_CUTOVER_REQUIRED_APPROVALS,
        'approvals' => array_values(array_intersect(APP_SQLITE_MYSQL_CUTOVER_REQUIRED_APPROVALS, array_keys($approvalSet))),
        'cutover' => [
...
/** @param array<string,mixed> $cutoverPlan @param array<string,mixed> $switch @param array<string,mixed> $rehearsal @return array<string,mixed> */
function app_sqlite_mysql_cutover_operator_package(array $cutoverPlan, array $switch, array $rehearsal, array $approvals = []): array
{
    $errors = [];
    ...
    $approvalSet = array_fill_keys(array_values(array_map('strval', $approvals)), true);
    foreach (APP_SQLITE_MYSQL_CUTOVER_OPERATOR_REQUIRED_APPROVALS as $approval) {
        if (!isset($approvalSet[$approval])) $errors[] = 'missing_approval:' . $approval;
    }

    $errors = array_values(array_unique($errors));
    $safe = [
        'operator_package_version' => APP_SQLITE_MYSQL_CUTOVER_OPERATOR_PACKAGE_VERSION,
        'operator_package_ready' => $errors === [],
        'stage' => $errors === [] ? 'operator_package_ready' : 'operator_package_blocked',
        'mutation_performed' => false,
        'cutover_contract_sha256' => (string) ($cutoverPlan['cutover_contract_sha256'] ?? ''),
        'requires_explicit_approval' => true,
        'required_approvals' => APP_SQLITE_MYSQL_CUTOVER_OPERATOR_REQUIRED_APPROVALS,
        'approvals' => array_values(array_intersect(APP_SQLITE_MYSQL_CUTOVER_OPERATOR_REQUIRED_APPROVALS, array_keys($approvalSet))),
        'switch' => [
...
function app_sqlite_mysql_cutover_token_valid(string $value): bool
{
    return preg_match('/^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$/', $value) === 1;
}

function app_sqlite_mysql_cutover_reference_valid(string $value): bool
{
    return $value !== '' && strlen($value) <= 200 && preg_match('/^[A-Za-z0-9][A-Za-z0-9_\/.:-]*$/', $value) === 1 && !str_contains($value, '://');
}

function app_sqlite_mysql_cutover_timestamp_valid(string $value): bool
{
    return preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$/', $value) === 1;
}
