 ...
 * }
 */
function app_project_db_access_function_update_delete_where_form_from_item(array $item): array
{
    return [
 ...
 * }
 */
function app_project_db_access_function_update_delete_where_form_apply_query_defaults(array $input): array
{
 ...
 * } $request
 */
function app_render_project_db_access_function_update_delete_where_edit_page(array $app, array $request): void
{
    ...
    $updateDeleteWhereKey = trim(app_route_param($request, 'update_delete_where_key'));
    $isNew = $updateDeleteWhereKey === '';

    if (!$isNew && !ctype_digit($updateDeleteWhereKey)) {
        app_render_bad_request_page($app, $request, 'update/delete where key の形式が不正です。');
        return;
    }
    ...
    $currentItemError = '';
    if (!$isNew) {
        $currentItemResult = app_fetch_db_access_function_update_delete_where_item(
            $app,
            $projectKey,
            $entity['source_name'],
            $method['name'],
            $updateDeleteWhereKey,
        );
    ...
    $input = $currentItem !== null
        ? app_project_db_access_function_update_delete_where_form_from_item($currentItem)
        : app_db_access_function_update_delete_where_form_defaults();
    if ($isNew && $catalogError === '' && $catalogItems !== []) {
        $input['where_order'] = (string) count($catalogItems);
    }
    if ($isNew) {
        $input = app_project_db_access_function_update_delete_where_form_apply_query_defaults($input);
    }
    ...
    if (app_request_method_is($request, 'POST')) {
        ...
        if (!app_verify_csrf_token(app_post_param('_csrf'))) {
        ...
        } elseif (!$isNew && trim(app_post_param('update_delete_where_id')) !== $updateDeleteWhereKey) {
            $errors[] = '更新対象の update/delete where key が route と一致しません。';
        } elseif ($formAction === 'delete') {
            if ($isNew) {
                $errors[] = '新規 row は削除できません。';
            } else {
                $deleteResult = app_delete_db_access_function_update_delete_where(
                    $app,
                    $projectKey,
                    $entity['source_name'],
                    $method['name'],
            ...
            $validation = app_validate_db_access_function_update_delete_where_form([
            ...
            if ($errors === []) {
                $errors = array_merge(
                    $errors,
                    app_project_db_access_validate_update_delete_where_metadata_refs(
                        $app,
                        $projectKey,
                        $targetTableName,
                        $input,
            ...
            if ($errors === []) {
                if ($isNew) {
                    $createResult = app_create_db_access_function_update_delete_where($app, [
                    ...
                    if ($createResult['ok']) {
                        app_send_redirect_response(
                            ...
                            . '/db-access/' . rawurlencode($entity['source_name'])
                            . '/functions/' . rawurlencode($method['name'])
                            . '/update-delete-where/' . rawurlencode($createResult['item_id'])
                            . '?updated=1',
                        );
                        return;
                    }
                    ...
                    $updateResult = app_update_db_access_function_update_delete_where($app, [
                        'project_key' => $projectKey,
                        'source_name' => $entity['source_name'],
                        'function_name' => $method['name'],
                        'update_delete_where_id' => $updateDeleteWhereKey,
                        'target_table_column_name' => $input['target_table_column_name'],
                        'parameter_type' => $input['parameter_type'],
                        'parameter_data_type' => $input['parameter_data_type'],
                        'fixed_parameter' => $input['fixed_parameter'],
                    ...
                    if ($updateResult['ok']) {
                        app_send_redirect_response(
                            ...
                            . '/db-access/' . rawurlencode($entity['source_name'])
                            . '/functions/' . rawurlencode($method['name'])
                            . '/update-delete-where/' . rawurlencode($updateDeleteWhereKey)
                            . '?updated=1',
                        );
                        return;
                    }
    ...
    $effectiveUpdateDeleteWhereKey = !$isNew ? $updateDeleteWhereKey : '';
...
<head>
    ...
    <title><?php echo app_h($app['site_name']); ?> - Project DB Access Function Update Delete Where Edit</title>
    <style>
        body {
...
<main>
    ...
    <div class="summary-grid">
        ...
        <section class="note-card">
            <h2>現在の制約</h2>
            <ul>
                <li>function detail の target table と canonical <code>dbtablecolumns</code> に対する column existence check は行うが、same-table OR group の意味整合チェックはまだ未実装</li>
                <li>input-aid は generated property 候補ベースの簡易版を先行実装した</li>
                <li>same-table OR group の grouping は保持するが、意味整合の自動検証はまだ未実装</li>
            </ul>
        </section>
    </div>

    <?php if ($updated): ?>
        <div class="success">update/delete where row を保存しました。</div>
    <?php endif; ?>
    ...
    <?php if ($canonicalFunctionItem !== null && $catalogError === '' && $currentItemError === ''): ?>
        ...
        echo $isNew
            ? '/projects/' . rawurlencode($projectKey) . '/db-access/' . rawurlencode($entity['source_name']) . '/functions/' . rawurlencode($method['name']) . '/update-delete-where/new'
            : '/projects/' . rawurlencode($projectKey) . '/db-access/' . rawurlencode($entity['source_name']) . '/functions/' . rawurlencode($method['name']) . '/update-delete-where/' . rawurlencode($effectiveUpdateDeleteWhereKey);
        ?>">
            <input type="hidden" name="_csrf" value="<?php echo app_h($csrfToken); ?>">
            <input type="hidden" name="update_delete_where_id" value="<?php echo app_h($effectiveUpdateDeleteWhereKey); ?>">
            ...
            <select id="parameter_type" name="parameter_type">
                <?php foreach (app_allowed_db_access_update_delete_where_parameter_types() as $parameterType): ?>
                    <option value="<?php echo app_h($parameterType); ?>"<?php echo $input['parameter_type'] === $parameterType ? ' selected' : ''; ?>>
                        <?php echo app_h(app_db_access_update_delete_where_parameter_type_caption($parameterType)); ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <div class="field-group" id="parameter_data_type_group">
                ...
                <select id="parameter_data_type" name="parameter_data_type">
                    <?php foreach (app_allowed_db_access_select_where_parameter_data_types() as $dataType): ?>
                        <option value="<?php echo app_h($dataType); ?>"<?php echo $input['parameter_data_type'] === $dataType ? ' selected' : ''; ?>>
    ...
    <section class="summary-grid">
        <section class="summary-card">
            ...
            <ul>
                <li>target column: <code><?php echo app_h($input['target_table_column_name'] !== '' ? $input['target_table_column_name'] : '(blank)'); ?></code></li>
                <li>relation: <code><?php echo app_h($input['relational_operator']); ?></code></li>
                <li>parameter type: <code><?php echo app_h(app_db_access_update_delete_where_parameter_type_caption($input['parameter_type'])); ?></code></li>
                <li>parameter summary: <code><?php echo app_h(app_db_access_update_delete_where_parameter_summary($input)); ?></code></li>
                <li>OR group: <code><?php echo app_h($input['or_group'] !== '' ? $input['or_group'] : '(blank)'); ?></code></li>
    ...
    <p><a href="/projects/<?php echo rawurlencode($projectKey); ?>/db-access/<?php echo rawurlencode($entity['source_name']); ?>/functions/<?php echo rawurlencode($method['name']); ?>/update-delete-where">Back to Update/Delete Where List</a></p>
</main>
<script>
function updateUpdateDeleteWhereFormVisibility() {
...
document.addEventListener('DOMContentLoaded', function () {
    const parameterType = document.getElementById('parameter_type');
    if (parameterType) {
        parameterType.addEventListener('change', updateUpdateDeleteWhereFormVisibility);
    }
    updateUpdateDeleteWhereFormVisibility();
});
</script>
</body>
</html>
