...
function app_project_data_classes_path(string $projectKey): string
{
    return '/projects/' . rawurlencode($projectKey) . '/data-classes';
}

function app_project_data_classes_sync_path(string $projectKey): string
{
    return app_project_data_classes_path($projectKey) . '/sync';
}

function app_project_data_class_detail_path(string $projectKey, string $dataClassKey): string
{
    return app_project_data_classes_path($projectKey) . '/' . rawurlencode($dataClassKey);
}

function app_project_data_class_edit_path(string $projectKey, string $dataClassKey): string
{
    return app_project_data_class_detail_path($projectKey, $dataClassKey) . '/edit';
}

function app_project_data_class_fields_path(string $projectKey, string $dataClassKey): string
{
    return app_project_data_class_detail_path($projectKey, $dataClassKey) . '/fields';
}

function app_project_data_class_field_new_path(string $projectKey, string $dataClassKey): string
{
    return app_project_data_class_fields_path($projectKey, $dataClassKey) . '/new';
}

function app_project_data_class_field_edit_path(
    string $projectKey,
    string $dataClassKey,
    string $fieldKey,
): string {
    return app_project_data_class_fields_path($projectKey, $dataClassKey)
        . '/' . rawurlencode($fieldKey) . '/edit';
}

function app_project_data_class_source_path(string $projectKey, string $dataClassKey): string
{
    return app_project_data_class_detail_path($projectKey, $dataClassKey) . '/source';
}
 ...
 * }|null
 */
function app_project_data_class_route_bootstrap(array $app, array $request, array $allowedMethods = ['GET']): ?array
{
    if ($app['site'] !== 'admin') {
    ...
    if (!app_auth_has_any_role(['admin', 'config'], $principal)) {
        app_render_forbidden_page($app, $request, 'data class の参照には admin または config role が必要です。');
        return null;
    }
