 ...
 * } $request
 */
function app_render_project_db_access_function_select_where_page(array $app, array $request): void
{
...
<head>
    ...
    <title><?php echo app_h($app['site_name']); ?> - Project DB Access Function Select Where</title>
    <style>
        body {
...
<main>
    ...
    <div class="summary-grid">
        ...
        <section class="note-card">
            ...
            <ul>
                ...
                <li>一覧は canonical row のみで、generated method から自動推定はしない</li>
                <li>Join や Another Field の妥当性は、後段で table metadata と付き合わせて検証する</li>
            </ul>
    ...
    <?php if ($created): ?>
        <div class="success">select where row を作成しました。</div>
    <?php endif; ?>

    <?php if ($deleted): ?>
        <div class="success">select where row を削除しました。</div>
    <?php endif; ?>

    <?php if ($canonicalFunctionItem !== null && $catalogError === ''): ?>
        <p>
            <a href="<?php echo app_h($basePath); ?>/select-where/new">Add New Select Where</a>
            / <a href="<?php echo app_h($basePath); ?>/select-where/input-aid">Input Aid</a>
            <?php if ($items !== []): ?>
                / <a href="<?php echo app_h($basePath); ?>/select-where/change-order">Change Order</a>
            <?php endif; ?>
        </p>
    <?php endif; ?>
        ...
        <table>
            ...
            <?php foreach ($items as $index => $item): ?>
                ...
                <tr>
                    ...
                    <td><code><?php echo app_h(app_db_access_select_where_join_type_caption($item['join_type'])); ?></code></td>
                    <td><code><?php echo app_h($targetLabel !== '' ? $targetLabel : '(blank)'); ?></code></td>
                    <td><code><?php echo app_h($item['relational_operator']); ?></code></td>
                    <td>
                        <code><?php echo app_h(app_db_access_select_where_parameter_type_caption($item['parameter_type'])); ?></code><br>
                    ...
                    <td><a href="/projects/<?php echo rawurlencode($projectKey); ?>/db-access/<?php echo rawurlencode($entity['source_name']); ?>/functions/<?php echo rawurlencode($method['name']); ?>/select-where/<?php echo rawurlencode($item['select_where_id']); ?>">edit</a></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <section class="summary-grid">
        ...
        <section class="note-card">
            ...
            <ul>
                ...
                <li><a href="<?php echo app_h($basePath); ?>/select-where/input-aid">input-aid</a></li>
                <?php if ($items !== []): ?>
                    <li><a href="<?php echo app_h($basePath); ?>/select-where/change-order">change-order</a></li>
                <?php endif; ?>
    ...
    <?php if ($legacySchema['dbaccess_excerpt'] !== ''): ?>
        <section>
            <h2>`dbaccess-dafuncselectwhere.php` Preview</h2>
