...
function app_config_db_bootstrap_firebird_convert_column_definition(string $definition): string
{
    $converted = preg_replace('/\s+AFTER\s+[A-Za-z0-9_]+$/i', '', trim($definition));
    if (!is_string($converted)) {
        $converted = trim($definition);
    ...
    if (preg_match('/^([A-Za-z0-9_]+)\s+BIGINT\s+UNSIGNED\s+NOT\s+NULL\s+AUTO_INCREMENT$/i', $converted, $matches) === 1) {
        return strtoupper($matches[1]) . ' BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL';
    }
    ...
    $replacements = [
        ...
        '/\bTEXT\b/i' => 'BLOB SUB_TYPE TEXT',
        '/\bMEDIUMTEXT\b/i' => 'BLOB SUB_TYPE TEXT',
    ...
    if (preg_match('/^([A-Za-z0-9_]+)\s+(.+)$/s', $converted, $matches) !== 1) {
        throw new RuntimeException('Firebird schema へ変換できない column definition です。');
    }

    $tail = $matches[2];
    if (preg_match('/\bBLOB\s+SUB_TYPE\s+TEXT\b/i', $tail) === 1) {
        $tail = preg_replace('/\s+DEFAULT\s+(?:\'[^\']*\'|-?[0-9]+)/i', '', $tail);
        if (!is_string($tail)) {
            $tail = $matches[2];
        }
    }
    ...
    $tail = preg_replace(
        '/\s+NOT\s+NULL\s+DEFAULT\s+((?:CURRENT_TIMESTAMP)|(?:\'[^\']*\')|(?:-?[0-9]+))/i',
        ' DEFAULT $1 NOT NULL',
        $tail,
    );
    if (!is_string($tail)) {
    ...
    $tail = preg_replace('/(?<!NOT)\s+NULL\b/i', '', $tail);
    if (!is_string($tail)) {
        $tail = $matches[2];
    }

    return strtoupper($matches[1]) . ' ' . $tail;
}

function app_config_db_bootstrap_firebird_convert_create_table_statement(string $statement): string
{
    if (preg_match('/^CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z0-9_]+)\s*\((.*)\)\s*ENGINE\s*=/is', trim($statement), $matches) !== 1) {
        throw new RuntimeException('Firebird schema へ変換できない CREATE TABLE です。');
    }
    ...
    foreach (app_config_db_bootstrap_split_top_level_csv($matches[2]) as $entry) {
        ...
        $convertedEntries[] = app_config_db_bootstrap_firebird_convert_column_definition($normalized);
    }
 ...
 * @return list<string>
 */
function app_config_db_bootstrap_firebird_prepare_statement(PDO $pdo, string $statement): array
{
    $trimmed = trim(preg_replace('/^\s*--.*$/m', '', $statement) ?? $statement);
    if ($trimmed === '') {
        return [];
    ...
    if (preg_match('/^CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z0-9_]+)\b/i', $trimmed, $matches) === 1) {
        $tableName = $matches[1];
        if (app_sql_table_exists($pdo, $tableName)) {
            return [];
        }

        return [app_config_db_bootstrap_firebird_convert_create_table_statement($trimmed)];
    }

    if (preg_match('/^ALTER\s+TABLE\s+([A-Za-z0-9_]+)\s+ADD\s+COLUMN\s+IF\s+NOT\s+EXISTS\s+(.+)$/is', $trimmed, $matches) === 1) {
        $tableName = $matches[1];
        $columnDefinition = app_config_db_bootstrap_firebird_convert_column_definition($matches[2]);
        $columnName = strtok($columnDefinition, " \t\r\n");
        if (is_string($columnName)) {
            $columnName = trim($columnName, '"');
        }
        ...
        return ['ALTER TABLE ' . strtoupper($tableName) . ' ADD ' . $columnDefinition];
    }
...
function app_config_db_bootstrap_required_columns(): array
{
    return [
        ...
        'project_table_keys' => ['table_pid', 'key_name', 'key_kind', 'source_of_truth'],
        'project_table_key_columns' => ['table_key_id', 'column_pid', 'ordinal_position'],
        'project_table_foreign_keys' => [
            'table_pid',
            'constraint_name',
            'referenced_table_pid',
            'on_update_action',
            'on_delete_action',
            'source_of_truth',
        ],
...
function app_config_db_bootstrap_exec_statement(PDO $pdo, string $statement, string $sqlFile): void
{
    try {
        $pdo->exec($statement);
    } catch (Throwable $throwable) {
        $excerpt = substr(preg_replace('/\s+/', ' ', trim($statement)) ?? trim($statement), 0, 500);
        throw new RuntimeException(
            sprintf(
                'config DB bootstrap SQL failed in %s: %s; statement: %s',
                basename($sqlFile),
    ...
    try {
        ...
        foreach ($sqlFiles as $sqlFile) {
            ...
            if ($dialect === 'sqlite') {
                ...
                foreach (app_config_db_bootstrap_split_sql_statements($contents) as $statement) {
                    foreach (app_config_db_bootstrap_sqlite_prepare_statement($pdo, $statement) as $sqliteStatement) {
                        app_config_db_bootstrap_exec_statement($pdo, $sqliteStatement, $sqlFile);
            ...
            if ($dialect === 'firebird') {
                $appliedStatementCount = 0;
                foreach (app_config_db_bootstrap_split_sql_statements($contents) as $statement) {
                    foreach (app_config_db_bootstrap_firebird_prepare_statement($pdo, $statement) as $firebirdStatement) {
                        app_config_db_bootstrap_exec_statement($pdo, $firebirdStatement, $sqlFile);
                        $appliedStatementCount++;
                    }
                }
                if ($appliedStatementCount > 0) {
            ...
            app_config_db_bootstrap_exec_statement($pdo, $contents, $sqlFile);
            $appliedFiles[] = basename($sqlFile);
        }
