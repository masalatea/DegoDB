...
function app_project_compare_output_settings_path(string $projectKey, string $compareOutputKey = ''): string
{
    $path = '/projects/' . rawurlencode($projectKey) . '/compare-output-settings';
    if ($compareOutputKey !== '') {
...
function app_project_compare_output_additional_paths_path(
    string $projectKey,
    string $compareOutputKey,
    string $additionalPathKey = '',
): string {
    $path = '/projects/' . rawurlencode($projectKey) . '/compare-output-settings/additional-paths';
    $query = 'compare_output_key=' . rawurlencode($compareOutputKey);
    if ($additionalPathKey !== '') {
        $query .= '&additional_path_key=' . rawurlencode($additionalPathKey);
    }

    return $path . '?' . $query;
}

function app_project_compare_output_route_bootstrap(
    array $app,
    array $request,
    ...
    if (!app_auth_has_any_role(['admin', 'config'], $principal)) {
        app_render_forbidden_page($app, $request, 'compare output setting の参照には admin または config role が必要です。');
        return null;
    }
...
<head>
    ...
    <title><?php echo app_h($app['site_name']); ?> - Compare Output Setting</title>
</head>
<body>
<main>
    <h1><?php echo app_h($app['site_name']); ?></h1>
    <p>compare output setting の読み込みに失敗しました。</p>
    <ul>
...
function app_project_compare_output_item_route_bootstrap(
    array $app,
    array $request,
    array $allowedMethods = ['GET'],
): ?array {
    $bootstrap = app_project_compare_output_route_bootstrap($app, $request, $allowedMethods);
    if ($bootstrap === null) {
        return null;
    }

    $compareOutputKey = app_normalize_compare_output_key(app_query_param('compare_output_key'));
    if ($compareOutputKey === '' || !app_compare_output_key_is_valid($compareOutputKey)) {
        app_render_bad_request_page($app, $request, 'compare output key の形式が不正です。');
        return null;
    }

    $itemResult = app_fetch_project_compare_output_item($app, $bootstrap['project_key'], $compareOutputKey);
    if (!$itemResult['ok']) {
        app_send_html_response_headers($request, 500);
        ?>
<!DOCTYPE html>
...
<main>
    <h1><?php echo app_h($app['site_name']); ?></h1>
    <p>compare output detail の読み込みに失敗しました。</p>
    <ul>
        <li>project key: <code><?php echo app_h($bootstrap['project_key']); ?></code></li>
        <li>compare output key: <code><?php echo app_h($compareOutputKey); ?></code></li>
        <li>request id: <code><?php echo app_h($request['request_id']); ?></code></li>
    ...
    $bootstrap['compare_output_key'] = $compareOutputKey;
    $bootstrap['compare_output'] = $itemResult['item'];
