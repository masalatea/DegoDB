...
function app_project_tables_path(string $projectKey): string
{
    return '/projects/' . rawurlencode($projectKey) . '/tables';
}

function app_project_table_detail_path(string $projectKey, string $tableKey): string
{
    return app_project_tables_path($projectKey) . '/' . rawurlencode($tableKey);
}

function app_project_table_edit_path(string $projectKey, string $tableKey): string
{
    return app_project_table_detail_path($projectKey, $tableKey) . '/edit';
}

function app_project_table_columns_path(string $projectKey, string $tableKey): string
{
    return app_project_table_detail_path($projectKey, $tableKey) . '/columns';
}

function app_project_table_column_new_path(string $projectKey, string $tableKey): string
{
    return app_project_table_columns_path($projectKey, $tableKey) . '/new';
}

function app_project_table_column_edit_path(string $projectKey, string $tableKey, string $columnKey): string
{
    return app_project_table_columns_path($projectKey, $tableKey) . '/' . rawurlencode($columnKey) . '/edit';
}
 ...
 * }|null
 */
function app_project_table_route_bootstrap(array $app, array $request, array $allowedMethods = ['GET']): ?array
{
    if ($app['site'] !== 'admin') {
    ...
    if (!app_auth_has_any_role(['admin', 'config'], $principal)) {
        app_render_forbidden_page($app, $request, 'table metadata の参照には admin または config role が必要です。');
        return null;
    }
...
<main>
    ...
    <p>table metadata の読み込みに失敗しました。</p>
    <ul>
