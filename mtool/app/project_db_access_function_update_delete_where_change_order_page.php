 ...
 * }
 */
function app_project_db_access_function_update_delete_where_change_order_submission(array $items): array
{
    $orderInputs = [];
    $expectedIds = [];
    foreach ($items as $item) {
        $orderInputs[$item['update_delete_where_id']] = $item['where_order'];
        $expectedIds[] = $item['update_delete_where_id'];
    }
    ...
    for ($i = 0, $count = count($rowIds); $i < $count; $i++) {
        ...
        if (!ctype_digit($rowId)) {
            $errors[] = 'update/delete where row id の形式が不正です。';
            continue;
        }
 ...
 * } $request
 */
function app_render_project_db_access_function_update_delete_where_change_order_page(array $app, array $request): void
{
    ...
    if (app_request_method_is($request, 'POST')) {
        ...
        if (!app_verify_csrf_token(app_post_param('_csrf'))) {
            ...
            $errors[] = '並び替える update/delete where row がありません。';
        } else {
            if ($formAction === 'reset') {
                $updates = [];
                foreach ($items as $index => $item) {
                    $orderInputs[$item['update_delete_where_id']] = (string) ($index + 1);
                    $updates[] = [
                        'update_delete_where_id' => $item['update_delete_where_id'],
                        'where_order' => (string) ($index + 1),
                    ];
                }
            } else {
                $submission = app_project_db_access_function_update_delete_where_change_order_submission($items);
            ...
            if ($errors === []) {
                $reorderResult = app_reorder_db_access_function_update_delete_where($app, [
...
<head>
    ...
    <title><?php echo app_h($app['site_name']); ?> - Project DB Access Function Update Delete Where Change Order</title>
    <style>
        body {
...
<main>
    ...
    <div class="summary-grid">
        ...
        <section class="note-card">
            ...
            <ul>
                ...
                <li>input aid や target column 妥当性検証は別タスク</li>
            </ul>
    ...
    <?php if ($updated): ?>
        <div class="success">update/delete where row の並び順を更新しました。</div>
    <?php endif; ?>

    <?php if ($reset): ?>
        <div class="success">update/delete where row の並び順を 1..n に再採番しました。</div>
    <?php endif; ?>
            ...
            <table>
                ...
                <?php foreach ($items as $index => $item): ?>
                    <tr>
                        ...
                        <td>
                            <input type="hidden" name="row_ids[]" value="<?php echo app_h($item['update_delete_where_id']); ?>">
                            <input name="row_orders[]" type="number" min="0" value="<?php echo app_h($orderInputs[$item['update_delete_where_id']] ?? $item['where_order']); ?>">
                        </td>
                        <td><a href="<?php echo app_h($listPath); ?>/<?php echo rawurlencode($item['update_delete_where_id']); ?>">edit</a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <section class="summary-grid">
        <section class="summary-card">
            <h2>Legacy Route</h2>
            <ul>
                <li>legacy screen: <code>da_func_update_delete_where_change_order.php</code></li>
                <li>legacy include: <code>da_func_update_delete_where_change_order_include.php</code></li>
            </ul>
        </section>
