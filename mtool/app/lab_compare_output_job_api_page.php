 ...
 * } $request
 */
function app_render_lab_compare_output_job_api_page(array $app, array $request): void
{
    ...
    if (!app_auth_has_any_role(['lab', 'admin'], $principal)) {
        app_send_json_response($request, [
            'ok' => false,
            'error' => 'compare output job の閲覧には lab または admin role が必要です。',
        ], 403);
        return;
    }
    ...
    if ($jobResult['item'] === null) {
        app_send_json_response($request, [
            'ok' => false,
            'error' => 'compare output job が見つかりません。',
        ], 404);
        return;
    }
