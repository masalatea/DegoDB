...
function app_cli_lang_res_residual_usage(): string
{
    return <<<TEXT
Usage:
  php mtool/scripts/debug/language_resource/inspect_language_resource_db_residual_rows.php [options]

Options:
  --project-key=KEY             target project key
  --legacy-project-pid=PID      legacy Project.PID fallback (default: 1 for MTOOL, else 0)
  --source-of-truths=CSV        residual source_of_truth filter (default: manual)
  --row-limit=N                 max rows per table section (default: 50)
...
function app_cli_lang_res_residual_apply_env(array $overrides): void
{
 ...
 */
function app_cli_lang_res_residual_source_clause(
    ...
    return ' AND ' . $columnExpression . ' IN (' . implode(', ', $placeholders) . ')';
}
 ...
 * }
 */
function app_cli_lang_res_residual_parse_args(array $argv): array
{
    $parsed = [
        'project_key' => '',
        'legacy_project_pid' => 0,
        'source_of_truths' => ['manual'],
        'row_limit' => 50,
        ...
        switch ($name) {
            ...
            case 'legacy-project-pid':
                $parsed['legacy_project_pid'] = ctype_digit($normalizedValue)
                    ...
                    : -1;
                break;
            case 'source-of-truths':
                $items = array_map('trim', explode(',', $normalizedValue));
                $items = array_values(array_filter(
                    $items,
                    static fn (string $item): bool => $item !== '',
                ));
                $parsed['source_of_truths'] = $items === [] ? ['manual'] : $items;
                break;
            case 'row-limit':
    ...
    return [
        ...
        'source_of_truths' => $parsed['source_of_truths'],
 ...
 */
function app_cli_lang_res_residual_app_env(array $parsed): array
{
    $env = [
        'APP_SITE' => 'admin',
        'APP_AUTH_MODE' => 'stub',
        ...
        'APP_AUTH_STUB_NAME' => 'Language Resource Residual Inspector',
 ...
 */
function app_cli_lang_res_residual_resources_by_legacy_pid(array $resources): array
{
    $mapped = [];
 ...
 * @return list<string>
 */
function app_cli_lang_res_residual_diff_assoc(array $left, array $right, array $keys): array
{
    $diffs = [];
    foreach ($keys as $key) {
 ...
 */
function app_cli_lang_res_residual_resource_file_match(
    array $dbRow,
    ...
    $diffFields = app_cli_lang_res_residual_diff_assoc(
        $dbComparable,
        $fileComparable,
        array_keys($dbComparable),
    );

    return [
        'status' => $diffFields === [] ? 'exact-match' : 'diff',
        'resource' => [
            'legacy_resource_pid' => (int) ($fileResource['legacy_resource_pid'] ?? 0),
            'legacy_group_pid' => (int) ($fileResource['legacy_group_pid'] ?? 0),
            'group_name' => (string) (($groupsByPid[(string) ($fileResource['legacy_group_pid'] ?? 0)]['name'] ?? '')),
            'resource_key' => (string) ($fileResource['resource_key'] ?? ''),
            'key_name' => (string) ($fileResource['key_name'] ?? ''),
        ],
 ...
 * @param array<string,array<string,mixed>> $languagesByPid
 * @return array<string,mixed>
 */
function app_cli_lang_res_residual_caption_file_match(
    array $dbRow,
    array $captionsByResourcePid,
    array $languagesByPid,
): array {
    $legacyResourcePid = (int) ($dbRow['legacy_resource_pid'] ?? 0);
    ...
    return [
        'status' => $diffFields === [] ? 'exact-match' : 'diff',
        'caption' => [
            'legacy_caption_pid' => (int) ($fileCaption['legacy_caption_pid'] ?? 0),
...
/**
 * @param list<string> $sourceOfTruths
 ...
 * }
 */
function app_cli_lang_res_residual_report(
    PDO $pdo,
    string $projectKey,
    array $sourceOfTruths,
    int $rowLimit,
    ...
    $resourceSourceClause = app_cli_lang_res_residual_source_clause(
        $sourceOfTruths,
        $resourceParams,
        'r.source_of_truth',
        'resource_source_',
    );
    $captionSourceClause = app_cli_lang_res_residual_source_clause(
        $sourceOfTruths,
        $captionParams,
        'c.source_of_truth',
        'caption_source_',
    ...
    $fileResourcesByLegacyPid = app_cli_lang_res_residual_resources_by_legacy_pid(
    ...
    $resourceStatement = $pdo->prepare(
        'SELECT
            r.id,
            r.project_language_resource_group_id,
        ...
        . $resourceSourceClause
        . '
        ORDER BY r.id
        LIMIT ' . (int) $rowLimit
    );
    $resourceStatement->execute($resourceParams);
    ...
    foreach ($resourceStatement->fetchAll() as $row) {
        ...
        $resource['file_match'] = app_cli_lang_res_residual_resource_file_match(
            $resource,
    ...
    $captionStatement = $pdo->prepare(
        'SELECT
            c.id,
            c.project_language_resource_id,
            c.project_language_resource_group_id,
    ...
    $captionStatement->execute($captionParams);
    ...
    foreach ($captionStatement->fetchAll() as $row) {
        ...
        $caption = [
            ...
            'legacy_language_pid' => $legacyLanguagePid,
            'locale_hint' => (string) ($language['filename_suffix'] ?? ''),
            'language_caption' => (string) ($language['caption'] ?? ''),
            'caption' => (string) ($row['caption'] ?? ''),
            'caption_auto_translated' => (string) ($row['caption_auto_translated'] ?? ''),
            'notes' => (string) ($row['notes'] ?? ''),
            'source_of_truth' => (string) ($row['source_of_truth'] ?? ''),
        ];
        $caption['file_match'] = app_cli_lang_res_residual_caption_file_match(
            $caption,
...
$parsed = app_cli_lang_res_residual_parse_args($argv);
if ($parsed['help']) {
    fwrite(STDOUT, app_cli_lang_res_residual_usage() . PHP_EOL);
    exit(0);
}

if (!$parsed['ok']) {
    fwrite(STDERR, $parsed['error'] . PHP_EOL . PHP_EOL . app_cli_lang_res_residual_usage() . PHP_EOL);
    exit(64);
}

try {
    app_cli_lang_res_residual_apply_env(
        app_cli_lang_res_residual_app_env($parsed),
    );
    ...
    $report = app_cli_lang_res_residual_report(
        $pdo,
        $parsed['project_key'],
        $parsed['source_of_truths'],
        $parsed['row_limit'],
        $catalogResult['item'],
    );
