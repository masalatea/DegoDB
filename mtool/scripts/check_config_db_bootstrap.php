 ...
 * }
 */
function app_cli_check_config_db_bootstrap_parse_args(array $argv): array
{
    $sqlDir = '';
    $requestedBy = 'cli';
...
function app_cli_check_config_db_bootstrap_usage(): string
{
    return <<<TEXT
Usage:
  php mtool/scripts/check_config_db_bootstrap.php
...
$parsed = app_cli_check_config_db_bootstrap_parse_args($argv);
if ($parsed['help']) {
    fwrite(STDOUT, app_cli_check_config_db_bootstrap_usage() . PHP_EOL);
    exit(0);
}

if (!$parsed['ok']) {
    fwrite(STDERR, $parsed['error'] . PHP_EOL . PHP_EOL . app_cli_check_config_db_bootstrap_usage() . PHP_EOL);
    exit(64);
}
