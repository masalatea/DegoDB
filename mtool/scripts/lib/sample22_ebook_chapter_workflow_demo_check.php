...
const APP_SAMPLE22_EBOOK_CHAPTER_WORKFLOW_OPENAPI_PATHS = [
    '/proxyserver-EbookWorkflowChapter-GetPublishedEbookWorkflowChapterList.php',
    '/proxyserver-EbookWorkflowChapter-GetPublishedEbookWorkflowChapter.php',
    '/proxyserver-EbookWorkflowChapter-InsertEbookWorkflowChapter.php',
    '/proxyserver-EbookWorkflowChapter-UpdateEbookWorkflowChapterDraft.php',
    '/proxyserver-EbookWorkflowChapter-UpdateEbookWorkflowChapterOrder.php',
    '/proxyserver-EbookWorkflowChapter-PublishEbookWorkflowChapter.php',
];

function app_sample22_ebook_chapter_workflow_default_reference_root(): string
{
    return app_sample_pack_reference_root('sample22-ebook-chapter-workflow-demo');
}

function app_sample22_ebook_chapter_workflow_assert_same(mixed $expected, mixed $actual, string $label, array &$errors): void
{
    if ($expected === $actual) {
        return;
    }
...
function app_sample22_ebook_chapter_workflow_tree_snapshot(string $root): array
{
...
function app_sample22_ebook_chapter_workflow_compare_file_sets(
...
function app_sample22_ebook_chapter_workflow_read_json_file(string $path): array
{
...
function app_sample22_ebook_chapter_workflow_publish_one(
    array $app,
    string $requestedBy,
    ...
    $sourceOutputResult = app_fetch_project_source_output_item(
        $app,
        APP_SAMPLE22_EBOOK_CHAPTER_WORKFLOW_PROJECT_KEY,
        $sourceOutputKey,
    );
    ...
    $expectedSnapshot = app_sample22_ebook_chapter_workflow_tree_snapshot($expectedRoot);
    $actualSnapshot = app_sample22_ebook_chapter_workflow_tree_snapshot($publishedRoot);
    ...
    $fileChecks = app_sample22_ebook_chapter_workflow_compare_file_sets(
        $sourceOutputKey,
    ...
    if ($sourceOutputKey === 'OPENAPI-JSON') {
        $openApiJsonResult = app_sample22_ebook_chapter_workflow_read_json_file($publishedRoot . '/openapi.json');
        ...
        app_sample22_ebook_chapter_workflow_assert_same('3.0.3', (string) ($spec['openapi'] ?? ''), 'openapi version', $errors);
        app_sample22_ebook_chapter_workflow_assert_same('Sample22 OpenAPI JSON', (string) ($spec['info']['title'] ?? ''), 'openapi info title', $errors);
        foreach (APP_SAMPLE22_EBOOK_CHAPTER_WORKFLOW_OPENAPI_PATHS as $path) {
            app_sample22_ebook_chapter_workflow_assert_same(true, is_array($spec['paths'][$path] ?? null), 'openapi path exists ' . $path, $errors);
        }
        app_sample22_ebook_chapter_workflow_assert_same(
            true,
            is_array($spec['components']['schemas']['EbookWorkflowChapterData'] ?? null),
            'EbookWorkflowChapterData schema exists',
            $errors,
        );
        $openApiSummary = [
...
function app_sample22_ebook_chapter_workflow_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $steps = [
        'table_imports' => [],
    ...
    foreach (APP_SAMPLE22_EBOOK_CHAPTER_WORKFLOW_TABLE_NAMES as $tableName) {
        ...
        app_sample22_ebook_chapter_workflow_assert_same(1, $tablePreview['summary']['source_table_count'], 'table source_table_count ' . $tableName, $errors);
    }

    $dataClassSync = app_project_data_class_sync_apply($app, APP_SAMPLE22_EBOOK_CHAPTER_WORKFLOW_PROJECT_KEY);
    ...
    $dataClassPreview = app_project_data_class_sync_preview($app, APP_SAMPLE22_EBOOK_CHAPTER_WORKFLOW_PROJECT_KEY);
    ...
    app_sample22_ebook_chapter_workflow_assert_same(3, $dataClassPreview['summary']['canonical_data_class_count'], 'data_class canonical_data_class_count', $errors);

    foreach (APP_SAMPLE22_EBOOK_CHAPTER_WORKFLOW_KEYS as $sourceOutputKey) {
        $outputResult = app_sample22_ebook_chapter_workflow_publish_one(
            $app,
            $requestedBy,
            $referenceRoot,
            $sourceOutputKey,
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $errors,
        'error' => $errors === [] ? '' : 'sample22 ebook chapter workflow verification failed.',
    ];
}
