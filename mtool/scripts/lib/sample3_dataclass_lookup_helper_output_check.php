...
function app_sample3_dataclass_default_reference_root(): string
{
    return app_sample_pack_reference_root('sample03-dataclass-lookup-and-helper');
}

function app_sample3_dataclass_assert_same(mixed $expected, mixed $actual, string $label, array &$errors): void
{
    if ($expected === $actual) {
        return;
    }
...
function app_sample3_dataclass_tree_snapshot(string $root): array
{
...
function app_sample3_dataclass_compare_file_sets(
    array $expectedFiles,
    array $actualFiles,
    string $label,
    array &$errors,
...
function app_sample3_dataclass_class_names(array $classes): array
{
    $names = [];
...
function app_sample3_dataclass_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $projectKey = APP_SAMPLE3_DATACLASS_PROJECT_KEY;
    $tableNames = APP_SAMPLE3_DATACLASS_TABLE_NAMES;
    $steps = [
        'table_imports' => [],
    ...
    foreach ($tableNames as $tableName) {
        ...
        app_sample3_dataclass_assert_same(
            1,
    ...
    app_sample3_dataclass_assert_same(
        $expectedClassNames,
        app_sample3_dataclass_class_names($dataClassPreview['classes']),
        'data_class class names',
        $assertionErrors,
    );

    foreach (APP_SAMPLE3_DATACLASS_REFERENCE_SOURCE_OUTPUT_KEYS as $sourceOutputKey) {
        ...
        $expectedSnapshot = app_sample3_dataclass_tree_snapshot($expectedRoot);
        $actualSnapshot = app_sample3_dataclass_tree_snapshot($publishedRoot);
        ...
        $fileChecks = app_sample3_dataclass_compare_file_sets(
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $assertionErrors,
        'error' => $ok ? '' : 'sample3 dataclass output check failed.',
    ];
}
