...
const APP_SAMPLE21_EBOOK_CATALOG_API_OPENAPI_PATHS = [
    '/proxyserver-EbookCatalogItem-GetPublicEbookCatalogList.php',
    '/proxyserver-EbookCatalogItem-GetPublicEbookBook.php',
];

function app_sample21_ebook_catalog_api_default_reference_root(): string
{
    return app_sample_pack_reference_root('sample21-ebook-catalog-api-demo');
}

function app_sample21_ebook_catalog_api_assert_same(mixed $expected, mixed $actual, string $label, array &$errors): void
{
    if ($expected === $actual) {
        return;
    }
...
function app_sample21_ebook_catalog_api_tree_snapshot(string $root): array
{
...
function app_sample21_ebook_catalog_api_compare_file_sets(
...
function app_sample21_ebook_catalog_api_read_json_file(string $path): array
{
...
function app_sample21_ebook_catalog_api_publish_one(
    array $app,
    string $requestedBy,
    ...
    $sourceOutputResult = app_fetch_project_source_output_item(
        $app,
        APP_SAMPLE21_EBOOK_CATALOG_API_PROJECT_KEY,
        $sourceOutputKey,
    );
    ...
    $expectedSnapshot = app_sample21_ebook_catalog_api_tree_snapshot($expectedRoot);
    $actualSnapshot = app_sample21_ebook_catalog_api_tree_snapshot($publishedRoot);
    ...
    $fileChecks = app_sample21_ebook_catalog_api_compare_file_sets(
        $sourceOutputKey,
    ...
    if ($sourceOutputKey === 'OPENAPI-JSON') {
        $openApiJsonResult = app_sample21_ebook_catalog_api_read_json_file($publishedRoot . '/openapi.json');
        ...
        app_sample21_ebook_catalog_api_assert_same('3.0.3', (string) ($spec['openapi'] ?? ''), 'openapi version', $errors);
        app_sample21_ebook_catalog_api_assert_same('Sample21 OpenAPI JSON', (string) ($spec['info']['title'] ?? ''), 'openapi info title', $errors);
        foreach (APP_SAMPLE21_EBOOK_CATALOG_API_OPENAPI_PATHS as $path) {
            app_sample21_ebook_catalog_api_assert_same(true, is_array($spec['paths'][$path] ?? null), 'openapi path exists ' . $path, $errors);
        }
        app_sample21_ebook_catalog_api_assert_same(
            true,
            is_array($spec['components']['schemas']['EbookCatalogItemData'] ?? null),
            'EbookCatalogItemData schema exists',
            $errors,
        );
        $openApiSummary = [
...
function app_sample21_ebook_catalog_api_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $steps = [
        'table_imports' => [],
    ...
    foreach (APP_SAMPLE21_EBOOK_CATALOG_API_TABLE_NAMES as $tableName) {
        ...
        app_sample21_ebook_catalog_api_assert_same(1, $tablePreview['summary']['source_table_count'], 'table source_table_count ' . $tableName, $errors);
    }

    $dataClassSync = app_project_data_class_sync_apply($app, APP_SAMPLE21_EBOOK_CATALOG_API_PROJECT_KEY);
    ...
    $dataClassPreview = app_project_data_class_sync_preview($app, APP_SAMPLE21_EBOOK_CATALOG_API_PROJECT_KEY);
    ...
    app_sample21_ebook_catalog_api_assert_same(7, $dataClassPreview['summary']['canonical_data_class_count'], 'data_class canonical_data_class_count', $errors);

    foreach (APP_SAMPLE21_EBOOK_CATALOG_API_KEYS as $sourceOutputKey) {
        $outputResult = app_sample21_ebook_catalog_api_publish_one(
            $app,
            $requestedBy,
            $referenceRoot,
            $sourceOutputKey,
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $errors,
        'error' => $errors === [] ? '' : 'sample21 ebook catalog API verification failed.',
    ];
}
