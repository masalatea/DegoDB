...
const APP_SAMPLE17_SPEC_CONTENT_DATA_CLASS_NAME = 'SpecContent';
const APP_SAMPLE17_SPEC_CONTENT_EXPECTED_METHOD_NAMES = [
    'GetDepthCaption',
];
const APP_SAMPLE17_SPEC_CONTENT_EXPECTED_WRAPPER_BOTTOM_FUNCTION = 'GetDepthCaptionCommon';

function app_sample17_speccontent_input_path(): string
{
    return app_runtime_storage_legacy_dbclasses_fixture_path('data-SpecContent.php');
}

function app_sample17_speccontent_output_root(): string
{
    return dirname(__DIR__, 3) . '/work/sample-packs/pattern09-top-level-declaration-single/output';
}

function app_sample17_speccontent_default_reference_root(): string
{
    return app_sample_pack_reference_root('pattern09-top-level-declaration-single');
}
 ...
 * }
 */
function app_sample17_speccontent_run(
    string $requestedBy,
    string $referenceRoot,
    bool $compareReference = true,
): array {
    $inputFile = app_sample17_speccontent_input_path();
    $outputRoot = app_sample17_speccontent_output_root();
    ...
    if ($inputFile === '' || !is_file($inputFile)) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample17 input file がありません。',
        ];
    }
    ...
    if (!$bootstrapInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample17 bootstrap info の取得に失敗しました: ' . $bootstrapInfo['error'],
        ];
    }
    ...
    if (!$migrationInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample17 migration info の取得に失敗しました: ' . $migrationInfo['error'],
        ];
    }
    ...
    app_reference_tree_assert_same(
        APP_SAMPLE17_SPEC_CONTENT_DATA_CLASS_NAME,
        $dataClassName,
        'sample17 data class name',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        1,
        (int) ($bootstrapInfo['class_count'] ?? 0),
        'sample17 class_count',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        true,
        (bool) ($bootstrapInfo['has_top_level_function'] ?? false),
        'sample17 has_top_level_function',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        APP_SAMPLE17_SPEC_CONTENT_EXPECTED_METHOD_NAMES,
        $bootstrapInfo['extra_method_names'] ?? [],
        'sample17 bootstrap extra_method_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [],
        $migrationInfo['wrapper_property_names'] ?? [],
        'sample17 wrapper_property_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        APP_SAMPLE17_SPEC_CONTENT_EXPECTED_METHOD_NAMES,
        $migrationInfo['wrapper_method_names'] ?? [],
        'sample17 wrapper_method_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [],
        $migrationInfo['generated_bottom_class_names'] ?? [],
        'sample17 generated_bottom_class_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [],
        $migrationInfo['generated_trailing_class_names'] ?? [],
        'sample17 generated_trailing_class_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        '',
        (string) ($migrationInfo['generated_base_additional_section'] ?? ''),
        'sample17 generated_base_additional_section',
        $assertionErrors,
    );
    ...
    app_reference_tree_assert_same(
        [APP_SAMPLE17_SPEC_CONTENT_EXPECTED_WRAPPER_BOTTOM_FUNCTION],
        $wrapperBottomFunctionNames,
        'sample17 wrapper bottom function names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        true,
    ...
    try {
        ...
        return [
            ...
            'error' => 'sample17 output の生成に失敗しました: ' . $throwable->getMessage(),
        ];
    }
    ...
    if ($compareReference) {
        if ($referenceRoot === '' || !is_dir($referenceRoot)) {
            return [
                ...
                'file_checks' => [],
                'assertion_errors' => $assertionErrors,
                'error' => 'sample17 reference root がありません。',
            ];
        }
    ...
    return [
        ...
        'assertion_errors' => $assertionErrors,
        'error' => $ok ? '' : 'sample17 top-level declaration verification failed',
    ];
}
