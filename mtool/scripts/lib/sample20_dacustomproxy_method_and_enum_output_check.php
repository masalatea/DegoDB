...
const APP_SAMPLE20_DA_CUSTOM_PROXY_DATA_CLASS_NAME = 'daCustomProxy';
const APP_SAMPLE20_DA_CUSTOM_PROXY_EXPECTED_METHOD_NAMES = [
    'IsLoginByLoginCookieToken',
];
const APP_SAMPLE20_DA_CUSTOM_PROXY_TRAILING_CLASS_NAMES = [
    'daCustomProxyAuthTypeEnum',
];

function app_sample20_dacustomproxy_input_path(): string
{
    return app_runtime_storage_legacy_dbclasses_fixture_path('data-daCustomProxy.php');
}

function app_sample20_dacustomproxy_output_root(): string
{
    return dirname(__DIR__, 3) . '/work/sample-packs/pattern12-method-and-enum-no-top-level/output';
}

function app_sample20_dacustomproxy_default_reference_root(): string
{
    return app_sample_pack_reference_root('pattern12-method-and-enum-no-top-level');
}
 ...
 * }
 */
function app_sample20_dacustomproxy_run(
    string $requestedBy,
    string $referenceRoot,
    bool $compareReference = true,
): array {
    $inputFile = app_sample20_dacustomproxy_input_path();
    $outputRoot = app_sample20_dacustomproxy_output_root();
    ...
    if ($inputFile === '' || !is_file($inputFile)) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample20 input file がありません。',
        ];
    }
    ...
    if (!$bootstrapInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample20 bootstrap info の取得に失敗しました: ' . $bootstrapInfo['error'],
        ];
    }
    ...
    if (!$migrationInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample20 migration info の取得に失敗しました: ' . $migrationInfo['error'],
        ];
    }
    ...
    app_reference_tree_assert_same(
        APP_SAMPLE20_DA_CUSTOM_PROXY_DATA_CLASS_NAME,
        $dataClassName,
        'sample20 data class name',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        2,
        (int) ($bootstrapInfo['class_count'] ?? 0),
        'sample20 class_count',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        false,
        (bool) ($bootstrapInfo['has_top_level_function'] ?? true),
        'sample20 has_top_level_function',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        APP_SAMPLE20_DA_CUSTOM_PROXY_EXPECTED_METHOD_NAMES,
        $bootstrapInfo['extra_method_names'] ?? [],
        'sample20 bootstrap extra_method_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [],
        $migrationInfo['wrapper_property_names'] ?? [],
        'sample20 wrapper_property_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        APP_SAMPLE20_DA_CUSTOM_PROXY_EXPECTED_METHOD_NAMES,
        $migrationInfo['wrapper_method_names'] ?? [],
        'sample20 wrapper_method_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        APP_SAMPLE20_DA_CUSTOM_PROXY_TRAILING_CLASS_NAMES,
        $migrationInfo['generated_trailing_class_names'] ?? [],
        'sample20 generated trailing class names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [],
        $migrationInfo['generated_bottom_class_names'] ?? [],
        'sample20 generated_bottom_class_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        true,
    ...
    try {
        ...
        return [
            ...
            'error' => 'sample20 output の生成に失敗しました: ' . $throwable->getMessage(),
        ];
    }
    ...
    if ($compareReference) {
        if ($referenceRoot === '' || !is_dir($referenceRoot)) {
            return [
                ...
                'file_checks' => [],
                'assertion_errors' => $assertionErrors,
                'error' => 'sample20 reference root がありません。',
            ];
        }
    ...
    return [
        ...
        'assertion_errors' => $assertionErrors,
        'error' => $ok ? '' : 'sample20 method-and-enum verification failed',
    ];
}
