...
const APP_SAMPLE2_DATACLASS_PROJECT_KEY = 'SAMPLE02';
const APP_SAMPLE2_DATACLASS_TABLE_NAME = 'task';
const APP_SAMPLE2_DATACLASS_REFERENCE_SOURCE_OUTPUT_KEYS = [
    'DATACLASS-PHP',
];

function app_sample2_dataclass_default_reference_root(): string
{
    return app_sample_pack_reference_root('sample02-dataclass-nullable-default-status');
}

/**
 * @param list<string> $errors
 */
function app_sample2_dataclass_assert_same(
    mixed $expected,
    mixed $actual,
    string $label,
    array &$errors,
 ...
 * }
 */
function app_sample2_dataclass_tree_snapshot(string $root): array
{
 ...
 * }>
 */
function app_sample2_dataclass_compare_file_sets(
    array $expectedFiles,
    array $actualFiles,
    string $label,
    array &$errors,
 ...
 * }
 */
function app_sample2_dataclass_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $projectKey = APP_SAMPLE2_DATACLASS_PROJECT_KEY;
    $tableName = APP_SAMPLE2_DATACLASS_TABLE_NAME;
    $steps = [
        'table_import' => null,
    ...
    app_sample2_dataclass_assert_same(
        1,
    ...
    foreach (APP_SAMPLE2_DATACLASS_REFERENCE_SOURCE_OUTPUT_KEYS as $sourceOutputKey) {
        ...
        $expectedSnapshot = app_sample2_dataclass_tree_snapshot($expectedRoot);
        $actualSnapshot = app_sample2_dataclass_tree_snapshot($publishedRoot);
        ...
        $fileChecks = app_sample2_dataclass_compare_file_sets(
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $assertionErrors,
        'error' => $ok ? '' : 'sample2 dataclass output check failed.',
    ];
}
