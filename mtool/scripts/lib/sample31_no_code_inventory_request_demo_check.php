...
const APP_SAMPLE31_NO_CODE_PROJECT_KEY = 'SAMPLE31';
const APP_SAMPLE31_NO_CODE_TABLE_NAME = 'inventory_request';
const APP_SAMPLE31_NO_CODE_SOURCE_OUTPUT_KEY = 'NO-CODE-RUNTIME';

/**
 * @param list<string> $errors
 */
function app_sample31_no_code_inventory_request_demo_assert_same(mixed $expected, mixed $actual, string $label, array &$errors): void
{
    if ($expected === $actual) {
        return;
    }
 ...
 */
function app_sample31_no_code_inventory_request_demo_read_json_file(string $path): array
{
    if (!is_file($path)) {
 ...
 * @return list<string>
 */
function app_sample31_no_code_inventory_request_demo_extract_names(array $items, string $key): array
{
 ...
 * }
 */
function app_sample31_no_code_inventory_request_demo_run(array $app, string $requestedBy): array
{
    $projectKey = APP_SAMPLE31_NO_CODE_PROJECT_KEY;
    $tableName = APP_SAMPLE31_NO_CODE_TABLE_NAME;
    $steps = [
        'table_import' => null,
    ...
    try {
        ...
        if (!$tableImport['ok']) {
            throw new RuntimeException('sample31 table import failed.');
        }
        ...
        if (!$dataClassSync['ok']) {
            throw new RuntimeException('sample31 data class sync failed.');
        }

        $sourceOutputResult = app_fetch_project_source_output_item(
            $app,
            $projectKey,
            APP_SAMPLE31_NO_CODE_SOURCE_OUTPUT_KEY,
        );
        if (!$sourceOutputResult['ok'] || $sourceOutputResult['item'] === null) {
            throw new RuntimeException(
                $sourceOutputResult['error'] !== ''
                    ? $sourceOutputResult['error']
                    : 'sample31 no-code source output definition was not found.',
            );
        }
        ...
        if (!$artifactResult['ok'] || $artifactResult['artifact'] === null) {
            throw new RuntimeException('sample31 no-code artifact generation failed: ' . $artifactResult['error']);
        }
        ...
        if (!$publishResult['ok'] || $publishResult['published'] === null) {
            throw new RuntimeException('sample31 no-code artifact publish failed: ' . $publishResult['error']);
        }
        $steps['published'] = $publishResult['published'];

        $publishedRoot = (string) ($publishResult['published']['published_root'] ?? '');
        $screenDefinitionJson = app_sample31_no_code_inventory_request_demo_read_json_file($publishedRoot . '/screen-definition.json');
        $runtimePreviewJson = app_sample31_no_code_inventory_request_demo_read_json_file($publishedRoot . '/runtime-preview.json');
        ...
        app_sample31_no_code_inventory_request_demo_assert_same('no-code-screen-definition-v0', $screenDefinition['definition_version'] ?? '', 'definition_version', $assertionErrors);
        app_sample31_no_code_inventory_request_demo_assert_same($projectKey, $screenDefinition['project_key'] ?? '', 'project_key', $assertionErrors);
        app_sample31_no_code_inventory_request_demo_assert_same(1, count($contracts), 'contract count', $assertionErrors);
        ...
        app_sample31_no_code_inventory_request_demo_assert_same(1, count($actions), 'action count', $assertionErrors);
        app_sample31_no_code_inventory_request_demo_assert_same('update_inventory_request', $actions[0]['action_key'] ?? '', 'action key', $assertionErrors);
        app_sample31_no_code_inventory_request_demo_assert_same('update', $actions[0]['operation_type'] ?? '', 'action operation_type', $assertionErrors);
        app_sample31_no_code_inventory_request_demo_assert_same('no-code-runtime-v0', $runtimePreview['runtime_version'] ?? '', 'runtime_version', $assertionErrors);
        app_sample31_no_code_inventory_request_demo_assert_same(3, count($runtimeScreens), 'runtime screen count', $assertionErrors);
        ...
        app_sample31_no_code_inventory_request_demo_assert_same(
            true,
            str_contains($runtimePreviewHtml, 'inventory_request_list')
        ...
        $steps['screen_definition'] = [
            ...
            'screen_types' => app_sample31_no_code_inventory_request_demo_extract_names($screens, 'screen_type'),
            'field_keys' => app_sample31_no_code_inventory_request_demo_extract_names($fields, 'field_key'),
    ...
    return [
        ...
        'error' => $assertionErrors === []
            ? ''
            : 'sample31 no-code inventory request demo assertions failed.',
    ];
}
