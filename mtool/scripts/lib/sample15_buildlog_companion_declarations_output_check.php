...
const APP_SAMPLE15_BUILD_LOG_DATA_CLASS_NAME = 'BuildLog';
const APP_SAMPLE15_BUILD_LOG_TRAILING_CLASS_NAMES = [
    'BuildLogMessageTypeEnum',
];

function app_sample15_buildlog_input_path(): string
{
    return app_runtime_storage_legacy_dbclasses_fixture_path('data-BuildLog.php');
}

function app_sample15_buildlog_output_root(): string
{
    return dirname(__DIR__, 3) . '/work/sample-packs/pattern06-companion-declarations-no-top-level/output';
}

function app_sample15_buildlog_default_reference_root(): string
{
    return app_sample_pack_reference_root('pattern06-companion-declarations-no-top-level');
}
 ...
 * }
 */
function app_sample15_buildlog_run(
    string $requestedBy,
    string $referenceRoot,
    bool $compareReference = true,
): array {
    $inputFile = app_sample15_buildlog_input_path();
    $outputRoot = app_sample15_buildlog_output_root();
    ...
    if ($inputFile === '' || !is_file($inputFile)) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample15 input file がありません。',
        ];
    }
    ...
    if (!$bootstrapInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample15 bootstrap info の取得に失敗しました: ' . $bootstrapInfo['error'],
        ];
    }
    ...
    if (!$migrationInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample15 migration info の取得に失敗しました: ' . $migrationInfo['error'],
        ];
    }
    ...
    app_reference_tree_assert_same(
        APP_SAMPLE15_BUILD_LOG_DATA_CLASS_NAME,
        $dataClassName,
        'sample15 data class name',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        2,
        (int) ($bootstrapInfo['class_count'] ?? 0),
        'sample15 class_count',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        false,
        !empty($bootstrapInfo['has_top_level_function']),
        'sample15 must keep no top level helper functions',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [],
        $migrationInfo['wrapper_property_names'] ?? [],
        'sample15 wrapper_property_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        APP_SAMPLE15_BUILD_LOG_TRAILING_CLASS_NAMES,
        $migrationInfo['generated_trailing_class_names'] ?? [],
        'sample15 generated trailing class names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        true,
    ...
    try {
        ...
        return [
            ...
            'error' => 'sample15 output の生成に失敗しました: ' . $throwable->getMessage(),
        ];
    }
    ...
    if ($compareReference) {
        if ($referenceRoot === '' || !is_dir($referenceRoot)) {
            return [
                ...
                'file_checks' => [],
                'assertion_errors' => $assertionErrors,
                'error' => 'sample15 reference root がありません。',
            ];
        }
    ...
    return [
        ...
        'assertion_errors' => $assertionErrors,
        'error' => $ok ? '' : 'sample15 BuildLog companion-declarations output check failed.',
    ];
}
