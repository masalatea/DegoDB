...
const APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_UPDATE_FUNCTION_NAME = 'UpdateSupportTicket';
const APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_DELETE_FUNCTION_NAME = 'DeleteSupportTicket';
const APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_FUNCTION_NAMES = [
...
const APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_REFERENCE_SOURCE_OUTPUT_KEYS = [
    'DATACLASS-PHP',
    'DBACCESS-PHP',
];
...
{
    return app_sample_pack_reference_root('sample10-dbaccess-mini-crud-flow');
}

function app_sample10_dbaccess_mini_crud_flow_assert_same(
    mixed $expected,
    mixed $actual,
    string $label,
    array &$errors,
...
function app_sample10_dbaccess_mini_crud_flow_tree_snapshot(string $root): array
{
...
function app_sample10_dbaccess_mini_crud_flow_compare_file_sets(
    array $expectedFiles,
    array $actualFiles,
    string $label,
    array &$errors,
...
function app_sample10_dbaccess_mini_crud_flow_extract_names(array $items, string $key): array
{
    $names = [];
...
function app_sample10_dbaccess_mini_crud_flow_function_expectations(): array
{
    return [
        APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_LIST_FUNCTION_NAME => [
...
function app_sample10_dbaccess_mini_crud_flow_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $projectKey = APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_PROJECT_KEY;
    $tableName = APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_TABLE_NAME;
    $sourceName = APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_SOURCE_NAME;
    $steps = [
        'table_import' => null,
    ...
    app_sample10_dbaccess_mini_crud_flow_assert_same(1, $tablePreview['summary']['source_table_count'], 'table source_table_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $tablePreview['summary']['table_insert_count'], 'table table_insert_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $tablePreview['summary']['table_changed_count'], 'table table_changed_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $tablePreview['summary']['table_delete_count'], 'table table_delete_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $tablePreview['summary']['column_insert_count'], 'table column_insert_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $tablePreview['summary']['column_update_count'], 'table column_update_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $tablePreview['summary']['column_delete_count'], 'table column_delete_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same($tableName, $tablePreview['tables'][0]['name'] ?? '', 'table preview table name', $assertionErrors);
    ...
    app_sample10_dbaccess_mini_crud_flow_assert_same(1, $dataClassPreview['summary']['table_count'], 'data_class table_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(1, $dataClassPreview['summary']['canonical_data_class_count'], 'data_class canonical_data_class_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $dataClassPreview['summary']['class_insert_count'], 'data_class class_insert_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $dataClassPreview['summary']['class_update_count'], 'data_class class_update_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $dataClassPreview['summary']['field_insert_count'], 'data_class field_insert_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $dataClassPreview['summary']['field_update_count'], 'data_class field_update_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $dataClassPreview['summary']['stale_class_count'], 'data_class stale_class_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(0, $dataClassPreview['summary']['stale_field_count'], 'data_class stale_field_count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(
        [$tableName],
        app_sample10_dbaccess_mini_crud_flow_extract_names($dataClassPreview['classes'], 'name'),
    ...
    app_sample10_dbaccess_mini_crud_flow_assert_same(1, count($classCatalogResult['items']), 'db_access class count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(
        [$sourceName],
        app_sample10_dbaccess_mini_crud_flow_extract_names($classCatalogResult['items'], 'source_name'),
        'db_access class names',
        $assertionErrors,
    );
    app_sample10_dbaccess_mini_crud_flow_assert_same('manual', $classCatalogResult['items'][0]['source_of_truth'] ?? '', 'db_access class source_of_truth', $assertionErrors);
    ...
    app_sample10_dbaccess_mini_crud_flow_assert_same(5, count($functionCatalogResult['items']), 'db_access function count', $assertionErrors);
    app_sample10_dbaccess_mini_crud_flow_assert_same(
        APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_FUNCTION_NAMES,
        app_sample10_dbaccess_mini_crud_flow_extract_names($functionCatalogResult['items'], 'function_name'),
        'db_access function names',
        $assertionErrors,
    );

    foreach (app_sample10_dbaccess_mini_crud_flow_function_expectations() as $functionName => $expected) {
        $functionStep = [
            'metadata' => null,
            'select_target_fields' => null,
            'select_wheres' => null,
            'insert_target_fields' => null,
            'update_target_fields' => null,
            'update_delete_wheres' => null,
        ];
        ...
        if (!$functionResult['ok'] || !is_array($functionResult['item'])) {
            ...
            return [
                ...
                'steps' => $steps,
                'assertion_errors' => $assertionErrors,
                'error' => 'db access function metadata の取得に失敗しました: ' . $functionName,
            ];
        }
        ...
        app_sample10_dbaccess_mini_crud_flow_assert_same($expected['sort_order_columns'], $functionResult['item']['sort_order_columns'] ?? '', 'db_access function sort_order_columns ' . $functionName, $assertionErrors);
        app_sample10_dbaccess_mini_crud_flow_assert_same($expected['limit_parameter_type'], $functionResult['item']['limit_parameter_type'] ?? '', 'db_access function limit_parameter_type ' . $functionName, $assertionErrors);
        app_sample10_dbaccess_mini_crud_flow_assert_same('manual', $functionResult['item']['source_of_truth'] ?? '', 'db_access function source_of_truth ' . $functionName, $assertionErrors);

        if ($expected['select_target_fields'] !== []) {
            $selectTargetFieldResult = app_fetch_db_access_function_select_target_field_catalog($app, $projectKey, $sourceName, $functionName);
            $functionStep['select_target_fields'] = [
            ...
            if (!$selectTargetFieldResult['ok']) {
                ...
                return [
                    ...
                    'steps' => $steps,
                    'assertion_errors' => $assertionErrors,
                    'error' => 'db access select target fields の取得に失敗しました: ' . $functionName,
                ];
            }

            app_sample10_dbaccess_mini_crud_flow_assert_same(
                $expected['select_target_fields'],
                app_sample10_dbaccess_mini_crud_flow_extract_names($selectTargetFieldResult['items'], 'target_table_column_name'),
                'db_access select target columns ' . $functionName,
                $assertionErrors,
            );
        }

        if ($expected['select_where_columns'] !== []) {
            $selectWhereResult = app_fetch_db_access_function_select_where_catalog($app, $projectKey, $sourceName, $functionName);
            $functionStep['select_wheres'] = [
            ...
            if (!$selectWhereResult['ok']) {
                ...
                return [
                    ...
                    'steps' => $steps,
                    'assertion_errors' => $assertionErrors,
                    'error' => 'db access select wheres の取得に失敗しました: ' . $functionName,
                ];
            }

            app_sample10_dbaccess_mini_crud_flow_assert_same(
                $expected['select_where_columns'],
                app_sample10_dbaccess_mini_crud_flow_extract_names($selectWhereResult['items'], 'target_table_column_name'),
                'db_access select where columns ' . $functionName,
                $assertionErrors,
            );
            app_sample10_dbaccess_mini_crud_flow_assert_same(
                array_fill(0, count($expected['select_where_columns']), 'argument'),
                app_sample10_dbaccess_mini_crud_flow_extract_names($selectWhereResult['items'], 'parameter_type'),
                'db_access select where parameter types ' . $functionName,
                $assertionErrors,
            );
        }

        if ($expected['target_field_mode'] === 'insert') {
            $insertTargetFieldResult = app_fetch_db_access_function_insert_target_field_catalog($app, $projectKey, $sourceName, $functionName);
            $functionStep['insert_target_fields'] = [
                'ok' => $insertTargetFieldResult['ok'],
                'items' => $insertTargetFieldResult['items'],
            ...
            if (!$insertTargetFieldResult['ok']) {
                ...
                return [
                    ...
                    'steps' => $steps,
                    'assertion_errors' => $assertionErrors,
                    'error' => 'db access insert target fields の取得に失敗しました: ' . $functionName,
                ];
            }

            app_sample10_dbaccess_mini_crud_flow_assert_same(
                APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_WRITE_TARGET_FIELDS,
                app_sample10_dbaccess_mini_crud_flow_extract_names($insertTargetFieldResult['items'], 'target_table_column_name'),
                'db_access insert target columns ' . $functionName,
                $assertionErrors,
            );
            app_sample10_dbaccess_mini_crud_flow_assert_same(
                array_fill(0, count(APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_WRITE_TARGET_FIELDS), 'argument'),
                app_sample10_dbaccess_mini_crud_flow_extract_names($insertTargetFieldResult['items'], 'parameter_type'),
                'db_access insert parameter types ' . $functionName,
                $assertionErrors,
            );
        }

        if ($expected['target_field_mode'] === 'update') {
            $updateTargetFieldResult = app_fetch_db_access_function_update_target_field_catalog($app, $projectKey, $sourceName, $functionName);
            $functionStep['update_target_fields'] = [
                'ok' => $updateTargetFieldResult['ok'],
                'items' => $updateTargetFieldResult['items'],
            ...
            if (!$updateTargetFieldResult['ok']) {
                ...
                return [
                    ...
                    'steps' => $steps,
                    'assertion_errors' => $assertionErrors,
                    'error' => 'db access update target fields の取得に失敗しました: ' . $functionName,
                ];
            }

            app_sample10_dbaccess_mini_crud_flow_assert_same(
                APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_WRITE_TARGET_FIELDS,
                app_sample10_dbaccess_mini_crud_flow_extract_names($updateTargetFieldResult['items'], 'target_table_column_name'),
                'db_access update target columns ' . $functionName,
                $assertionErrors,
            );
            app_sample10_dbaccess_mini_crud_flow_assert_same(
                array_fill(0, count(APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_WRITE_TARGET_FIELDS), 'argument'),
                app_sample10_dbaccess_mini_crud_flow_extract_names($updateTargetFieldResult['items'], 'parameter_type'),
                'db_access update parameter types ' . $functionName,
                $assertionErrors,
            );
        }

        if ($expected['where_count'] > 0) {
            $updateDeleteWhereResult = app_fetch_db_access_function_update_delete_where_catalog($app, $projectKey, $sourceName, $functionName);
            $functionStep['update_delete_wheres'] = [
                'ok' => $updateDeleteWhereResult['ok'],
                'items' => $updateDeleteWhereResult['items'],
            ...
            if (!$updateDeleteWhereResult['ok']) {
                ...
                return [
                    ...
                    'steps' => $steps,
                    'assertion_errors' => $assertionErrors,
                    'error' => 'db access update/delete wheres の取得に失敗しました: ' . $functionName,
                ];
            }
            ...
            app_sample10_dbaccess_mini_crud_flow_assert_same(
                ['argument'],
                app_sample10_dbaccess_mini_crud_flow_extract_names($updateDeleteWhereResult['items'], 'parameter_type'),
                'db_access update/delete where parameter types ' . $functionName,
                $assertionErrors,
            );
        }
    ...
    foreach (APP_SAMPLE10_DBACCESS_MINI_CRUD_FLOW_REFERENCE_SOURCE_OUTPUT_KEYS as $sourceOutputKey) {
        ...
        $expectedSnapshot = app_sample10_dbaccess_mini_crud_flow_tree_snapshot($expectedRoot);
        $actualSnapshot = app_sample10_dbaccess_mini_crud_flow_tree_snapshot($publishedRoot);
        ...
        $fileChecks = app_sample10_dbaccess_mini_crud_flow_compare_file_sets(
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $assertionErrors,
        'error' => $ok ? '' : 'sample10 dbaccess mini crud flow output check failed.',
    ];
}
