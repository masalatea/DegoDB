...
const APP_SAMPLE20_CONTENT_PUBLISHING_OPENAPI_PATHS = [
    '/proxyserver-ContentArticle-GetPublishedContentArticleList.php',
    '/proxyserver-ContentArticle-GetPublishedContentArticle.php',
];

function app_sample20_content_publishing_default_reference_root(): string
{
    return app_sample_pack_reference_root('sample20-content-publishing-demo');
}

function app_sample20_content_publishing_assert_same(mixed $expected, mixed $actual, string $label, array &$errors): void
{
    if ($expected === $actual) {
        return;
    }
...
function app_sample20_content_publishing_tree_snapshot(string $root): array
{
...
function app_sample20_content_publishing_compare_file_sets(
...
function app_sample20_content_publishing_read_json_file(string $path): array
{
...
function app_sample20_content_publishing_publish_one(
    array $app,
    string $requestedBy,
    ...
    $sourceOutputResult = app_fetch_project_source_output_item(
        $app,
        APP_SAMPLE20_CONTENT_PUBLISHING_PROJECT_KEY,
        $sourceOutputKey,
    );
    ...
    $expectedSnapshot = app_sample20_content_publishing_tree_snapshot($expectedRoot);
    $actualSnapshot = app_sample20_content_publishing_tree_snapshot($publishedRoot);
    ...
    $fileChecks = app_sample20_content_publishing_compare_file_sets(
        $sourceOutputKey,
    ...
    if ($sourceOutputKey === 'OPENAPI-JSON') {
        $openApiJsonResult = app_sample20_content_publishing_read_json_file($publishedRoot . '/openapi.json');
        ...
        app_sample20_content_publishing_assert_same('3.0.3', (string) ($spec['openapi'] ?? ''), 'openapi version', $errors);
        app_sample20_content_publishing_assert_same('Sample20 OpenAPI JSON', (string) ($spec['info']['title'] ?? ''), 'openapi info title', $errors);
        foreach (APP_SAMPLE20_CONTENT_PUBLISHING_OPENAPI_PATHS as $path) {
            app_sample20_content_publishing_assert_same(true, is_array($spec['paths'][$path] ?? null), 'openapi path exists ' . $path, $errors);
        }
        app_sample20_content_publishing_assert_same(
            true,
            is_array($spec['components']['schemas']['ContentArticleData'] ?? null),
            'ContentArticleData schema exists',
            $errors,
        );
        $openApiSummary = [
...
function app_sample20_content_publishing_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $steps = [
        'table_import' => null,
    ...
    $tableImport = app_project_table_import_apply(
        ...
        APP_SAMPLE20_CONTENT_PUBLISHING_PROJECT_KEY,
        'live-schema',
        APP_SAMPLE20_CONTENT_PUBLISHING_TABLE_NAME,
    );
    $steps['table_import'] = [
        'ok' => $tableImport['ok'],
    ...
    app_sample20_content_publishing_assert_same(1, $tablePreview['summary']['source_table_count'], 'table source_table_count', $errors);

    $dataClassSync = app_project_data_class_sync_apply($app, APP_SAMPLE20_CONTENT_PUBLISHING_PROJECT_KEY);
    ...
    $dataClassPreview = app_project_data_class_sync_preview($app, APP_SAMPLE20_CONTENT_PUBLISHING_PROJECT_KEY);
    ...
    app_sample20_content_publishing_assert_same(1, $dataClassPreview['summary']['canonical_data_class_count'], 'data_class canonical_data_class_count', $errors);

    foreach (APP_SAMPLE20_CONTENT_PUBLISHING_KEYS as $sourceOutputKey) {
        $outputResult = app_sample20_content_publishing_publish_one(
            $app,
            $requestedBy,
            $referenceRoot,
            $sourceOutputKey,
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $errors,
        'error' => $errors === [] ? '' : 'sample20 content publishing verification failed.',
    ];
}
