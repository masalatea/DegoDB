...
const APP_SAMPLE10_COMPARE_OUTPUT_DATA_CLASS_NAME = 'CompareOutput';
const APP_SAMPLE10_COMPARE_OUTPUT_TRAILING_CLASS_NAME = 'CompareOutputOutputFileTypeEnum';

function app_sample10_compare_output_input_path(): string
{
    return app_runtime_storage_legacy_dbclasses_fixture_path('data-CompareOutput.php');
}

function app_sample10_compare_output_output_root(): string
{
    return dirname(__DIR__, 3) . '/work/sample-packs/pattern05-companion-declarations-basic/output';
}

function app_sample10_compare_output_default_reference_root(): string
{
    return app_sample_pack_reference_root('pattern05-companion-declarations-basic');
}
 ...
 * }
 */
function app_sample10_compare_output_run(
    string $requestedBy,
    string $referenceRoot,
    bool $compareReference = true,
): array {
    $inputFile = app_sample10_compare_output_input_path();
    $outputRoot = app_sample10_compare_output_output_root();
    ...
    if ($inputFile === '' || !is_file($inputFile)) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample10 input file がありません。',
        ];
    }
    ...
    if (!$bootstrapInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample10 bootstrap info の取得に失敗しました: ' . $bootstrapInfo['error'],
        ];
    }
    ...
    if (!$migrationInfo['ok']) {
        return [
            ...
            'file_checks' => [],
            'assertion_errors' => [],
            'error' => 'sample10 migration info の取得に失敗しました: ' . $migrationInfo['error'],
        ];
    }
    ...
    app_reference_tree_assert_same(
        APP_SAMPLE10_COMPARE_OUTPUT_DATA_CLASS_NAME,
        $dataClassName,
        'sample10 data class name',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        2,
        (int) ($bootstrapInfo['class_count'] ?? 0),
        'sample10 class_count',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        true,
        !empty($bootstrapInfo['has_top_level_function']),
        'sample10 must keep top level helper in wrapper bottom area',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [],
        $migrationInfo['wrapper_property_names'] ?? [],
        'sample10 wrapper_property_names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        [APP_SAMPLE10_COMPARE_OUTPUT_TRAILING_CLASS_NAME],
        $migrationInfo['generated_trailing_class_names'] ?? [],
        'sample10 generated trailing class names',
        $assertionErrors,
    );
    app_reference_tree_assert_same(
        true,
    ...
    try {
        ...
        return [
            ...
            'error' => 'sample10 output の生成に失敗しました: ' . $throwable->getMessage(),
        ];
    }
    ...
    if ($compareReference) {
        if ($referenceRoot === '' || !is_dir($referenceRoot)) {
            return [
                ...
                'file_checks' => [],
                'assertion_errors' => $assertionErrors,
                'error' => 'sample10 reference root がありません。',
            ];
        }
    ...
    return [
        ...
        'assertion_errors' => $assertionErrors,
        'error' => $ok ? '' : 'sample10 CompareOutput companion-declarations output check failed.',
    ];
}
