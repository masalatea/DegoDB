...
require_once dirname(__DIR__, 2) . '/app/lab_swagger_service.php';
...
const APP_SAMPLE13_OPENAPI_EXPECTED_PATHS = [
    ...
    '/proxyserver-ApiTask-GetApiTask.php',
];

function app_sample13_openapi_default_reference_root(): string
{
    return app_sample_pack_reference_root('sample13-openapi-api-surface');
}

function app_sample13_openapi_assert_same(mixed $expected, mixed $actual, string $label, array &$errors): void
{
    if ($expected === $actual) {
        return;
    }
...
function app_sample13_openapi_tree_snapshot(string $root): array
{
...
function app_sample13_openapi_compare_file_sets(array $expectedFiles, array $actualFiles, array &$errors): array
{
    $expectedByPath = [];
    ...
    foreach ($paths as $relativePath) {
        ...
        if (!$expectedExists) {
            $errors[] = APP_SAMPLE13_OPENAPI_SOURCE_OUTPUT_KEY . ' unexpected extra file: ' . $relativePath;
        } elseif (!$actualExists) {
            $errors[] = APP_SAMPLE13_OPENAPI_SOURCE_OUTPUT_KEY . ' missing file: ' . $relativePath;
        } elseif ($expectedSha256 !== $actualSha256) {
            $errors[] = APP_SAMPLE13_OPENAPI_SOURCE_OUTPUT_KEY . ' digest mismatch: ' . $relativePath
...
function app_sample13_openapi_read_json_file(string $path): array
{
...
function app_sample13_openapi_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $steps = [
        'table_import' => null,
    ...
    $tableImport = app_project_table_import_apply(
        $app,
        APP_SAMPLE13_OPENAPI_PROJECT_KEY,
        'live-schema',
        APP_SAMPLE13_OPENAPI_TABLE_NAME,
    );
    $steps['table_import'] = [
        'ok' => $tableImport['ok'],
    ...
    app_sample13_openapi_assert_same(1, $tablePreview['summary']['source_table_count'], 'table source_table_count', $errors);

    $dataClassSync = app_project_data_class_sync_apply($app, APP_SAMPLE13_OPENAPI_PROJECT_KEY);
    ...
    $dataClassPreview = app_project_data_class_sync_preview($app, APP_SAMPLE13_OPENAPI_PROJECT_KEY);
    ...
    app_sample13_openapi_assert_same(1, $dataClassPreview['summary']['canonical_data_class_count'], 'data_class canonical_data_class_count', $errors);

    $sourceOutputResult = app_fetch_project_source_output_item(
        $app,
        APP_SAMPLE13_OPENAPI_PROJECT_KEY,
        APP_SAMPLE13_OPENAPI_SOURCE_OUTPUT_KEY,
    );
    ...
    $expectedRoot = $referenceRoot . '/' . APP_SAMPLE13_OPENAPI_SOURCE_OUTPUT_KEY;
    $expectedSnapshot = app_sample13_openapi_tree_snapshot($expectedRoot);
    $actualSnapshot = app_sample13_openapi_tree_snapshot($publishedRoot);
    ...
    $fileChecks = app_sample13_openapi_compare_file_sets($expectedSnapshot['files'], $actualSnapshot['files'], $errors);

    $openApiJsonResult = app_sample13_openapi_read_json_file($publishedRoot . '/openapi.json');
    ...
    app_sample13_openapi_assert_same('3.0.3', (string) ($spec['openapi'] ?? ''), 'openapi version', $errors);
    app_sample13_openapi_assert_same('Sample13 OpenAPI JSON', (string) ($spec['info']['title'] ?? ''), 'openapi info title', $errors);
    foreach (APP_SAMPLE13_OPENAPI_EXPECTED_PATHS as $path) {
        app_sample13_openapi_assert_same(true, is_array($spec['paths'][$path] ?? null), 'openapi path exists ' . $path, $errors);
    }
    app_sample13_openapi_assert_same(true, is_array($spec['components']['schemas']['ApiTaskData'] ?? null), 'ApiTaskData schema exists', $errors);

    $swaggerSpecResult = app_lab_swagger_resolve_spec(
        $app,
        APP_SAMPLE13_OPENAPI_PROJECT_KEY,
        $sourceOutputResult['item'],
    );
    $steps['swagger_spec'] = [
        'ok' => $swaggerSpecResult['ok'],
        'spec_source' => $swaggerSpecResult['spec_source'] ?? '',
        'error' => $swaggerSpecResult['error'] ?? '',
    ];
    if (!$swaggerSpecResult['ok']) {
        return [
            'ok' => false,
            'steps' => $steps,
            'assertion_errors' => $errors,
            'error' => 'swagger spec resolve に失敗しました: ' . $swaggerSpecResult['error'],
        ];
    }

    $steps['output'] = [
        'source_output_key' => APP_SAMPLE13_OPENAPI_SOURCE_OUTPUT_KEY,
        ...
        'openapi_paths' => array_keys($spec['paths'] ?? []),
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $errors,
        'error' => $errors === [] ? '' : 'sample13 OpenAPI API surface verification failed.',
    ];
}
