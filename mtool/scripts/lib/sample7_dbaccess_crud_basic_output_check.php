...
const APP_SAMPLE7_DBACCESS_CRUD_BASIC_FUNCTION_NAMES = [
    ...
    APP_SAMPLE7_DBACCESS_CRUD_BASIC_DELETE_FUNCTION_NAME,
];
const APP_SAMPLE7_DBACCESS_CRUD_BASIC_REFERENCE_SOURCE_OUTPUT_KEYS = [
    'DATACLASS-PHP',
    'DBACCESS-PHP',
];
...
{
    return app_sample_pack_reference_root('sample07-dbaccess-crud-basic');
}

function app_sample7_dbaccess_crud_basic_assert_same(
    mixed $expected,
    mixed $actual,
    string $label,
    array &$errors,
...
function app_sample7_dbaccess_crud_basic_tree_snapshot(string $root): array
{
...
function app_sample7_dbaccess_crud_basic_compare_file_sets(
    array $expectedFiles,
    array $actualFiles,
    string $label,
    array &$errors,
...
function app_sample7_dbaccess_crud_basic_extract_names(array $items, string $key): array
{
    $names = [];
...
function app_sample7_dbaccess_crud_basic_function_expectations(): array
{
    return [
        APP_SAMPLE7_DBACCESS_CRUD_BASIC_INSERT_FUNCTION_NAME => [
            ...
            'detected_signature' => 'public function InsertTodoItem($TodoItemObj)',
        ...
        APP_SAMPLE7_DBACCESS_CRUD_BASIC_UPDATE_FUNCTION_NAME => [
            'action_type' => 'UPDATE',
            'parameter_type' => 'classobject',
            'detected_signature' => 'public function UpdateTodoItem($TodoItemObj)',
        ...
        APP_SAMPLE7_DBACCESS_CRUD_BASIC_DELETE_FUNCTION_NAME => [
            'action_type' => 'DELETE',
            'parameter_type' => 'classobject',
            'detected_signature' => 'public function DeleteTodoItem($TodoItemObj)',
            'target_field_mode' => '',
            'where_count' => 1,
        ],
    ];
...
function app_sample7_dbaccess_crud_basic_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $projectKey = APP_SAMPLE7_DBACCESS_CRUD_BASIC_PROJECT_KEY;
    $tableName = APP_SAMPLE7_DBACCESS_CRUD_BASIC_TABLE_NAME;
    $sourceName = APP_SAMPLE7_DBACCESS_CRUD_BASIC_SOURCE_NAME;
    $steps = [
        'table_import' => null,
    ...
    app_sample7_dbaccess_crud_basic_assert_same(1, $tablePreview['summary']['source_table_count'], 'table source_table_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $tablePreview['summary']['table_insert_count'], 'table table_insert_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $tablePreview['summary']['table_changed_count'], 'table table_changed_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $tablePreview['summary']['table_delete_count'], 'table table_delete_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $tablePreview['summary']['column_insert_count'], 'table column_insert_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $tablePreview['summary']['column_update_count'], 'table column_update_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $tablePreview['summary']['column_delete_count'], 'table column_delete_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same($tableName, $tablePreview['tables'][0]['name'] ?? '', 'table preview table name', $assertionErrors);
    ...
    app_sample7_dbaccess_crud_basic_assert_same(1, $dataClassPreview['summary']['table_count'], 'data_class table_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(1, $dataClassPreview['summary']['canonical_data_class_count'], 'data_class canonical_data_class_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $dataClassPreview['summary']['class_insert_count'], 'data_class class_insert_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $dataClassPreview['summary']['class_update_count'], 'data_class class_update_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $dataClassPreview['summary']['field_insert_count'], 'data_class field_insert_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $dataClassPreview['summary']['field_update_count'], 'data_class field_update_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $dataClassPreview['summary']['stale_class_count'], 'data_class stale_class_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(0, $dataClassPreview['summary']['stale_field_count'], 'data_class stale_field_count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(
        [$tableName],
        app_sample7_dbaccess_crud_basic_extract_names($dataClassPreview['classes'], 'name'),
    ...
    app_sample7_dbaccess_crud_basic_assert_same(1, count($classCatalogResult['items']), 'db_access class count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same([$sourceName], app_sample7_dbaccess_crud_basic_extract_names($classCatalogResult['items'], 'source_name'), 'db_access class names', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same('manual', $classCatalogResult['items'][0]['source_of_truth'] ?? '', 'db_access class source_of_truth', $assertionErrors);
    ...
    app_sample7_dbaccess_crud_basic_assert_same(3, count($functionCatalogResult['items']), 'db_access function count', $assertionErrors);
    app_sample7_dbaccess_crud_basic_assert_same(
        APP_SAMPLE7_DBACCESS_CRUD_BASIC_FUNCTION_NAMES,
        app_sample7_dbaccess_crud_basic_extract_names($functionCatalogResult['items'], 'function_name'),
        'db_access function names',
        $assertionErrors,
    );

    foreach (app_sample7_dbaccess_crud_basic_function_expectations() as $functionName => $expected) {
        $functionStep = [
            'metadata' => null,
        ...
        app_sample7_dbaccess_crud_basic_assert_same('manual', $functionResult['item']['source_of_truth'] ?? '', 'db_access function source_of_truth ' . $functionName, $assertionErrors);
        ...
        if ($expected['target_field_mode'] === 'insert') {
            ...
            app_sample7_dbaccess_crud_basic_assert_same(
                APP_SAMPLE7_DBACCESS_CRUD_BASIC_WRITE_TARGET_FIELDS,
                app_sample7_dbaccess_crud_basic_extract_names($insertTargetFieldResult['items'], 'target_table_column_name'),
                'db_access insert target columns ' . $functionName,
                $assertionErrors,
            );
            app_sample7_dbaccess_crud_basic_assert_same(
                ['argument', 'argument', 'argument'],
                app_sample7_dbaccess_crud_basic_extract_names($insertTargetFieldResult['items'], 'parameter_type'),
        ...
        if ($expected['target_field_mode'] === 'update') {
            ...
            app_sample7_dbaccess_crud_basic_assert_same(
                APP_SAMPLE7_DBACCESS_CRUD_BASIC_WRITE_TARGET_FIELDS,
                app_sample7_dbaccess_crud_basic_extract_names($updateTargetFieldResult['items'], 'target_table_column_name'),
                'db_access update target columns ' . $functionName,
                $assertionErrors,
            );
            app_sample7_dbaccess_crud_basic_assert_same(
                ['argument', 'argument', 'argument'],
                app_sample7_dbaccess_crud_basic_extract_names($updateTargetFieldResult['items'], 'parameter_type'),
    ...
    foreach (APP_SAMPLE7_DBACCESS_CRUD_BASIC_REFERENCE_SOURCE_OUTPUT_KEYS as $sourceOutputKey) {
        ...
        $expectedSnapshot = app_sample7_dbaccess_crud_basic_tree_snapshot($expectedRoot);
        $actualSnapshot = app_sample7_dbaccess_crud_basic_tree_snapshot($publishedRoot);
        ...
        app_sample7_dbaccess_crud_basic_assert_same(
            $expectedSnapshot['file_count'],
            $actualSnapshot['file_count'],
            $sourceOutputKey . ' file_count',
            $assertionErrors,
        );
        app_sample7_dbaccess_crud_basic_assert_same(
            $expectedSnapshot['total_bytes'],
            $actualSnapshot['total_bytes'],
            $sourceOutputKey . ' total_bytes',
            $assertionErrors,
        );

        $fileChecks = app_sample7_dbaccess_crud_basic_compare_file_sets(
    ...
    return [
        ...
        'error' => $assertionErrors === []
            ? ''
            : 'sample07 dbaccess crud basic assertions failed.',
    ];
}
