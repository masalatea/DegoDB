...
{
    return app_sample_pack_reference_root('sample08-dbaccess-join-read-model');
}

function app_sample8_dbaccess_join_read_model_assert_same(
    mixed $expected,
    mixed $actual,
    string $label,
    array &$errors,
...
function app_sample8_dbaccess_join_read_model_tree_snapshot(string $root): array
{
...
function app_sample8_dbaccess_join_read_model_compare_file_sets(
    array $expectedFiles,
    array $actualFiles,
    string $label,
    array &$errors,
...
function app_sample8_dbaccess_join_read_model_extract_names(array $items, string $key): array
{
    $names = [];
...
function app_sample8_dbaccess_join_read_model_select_target_labels(array $items): array
{
    $labels = [];
    foreach ($items as $item) {
        if (!is_array($item)) {
...
function app_sample8_dbaccess_join_read_model_run(array $app, string $requestedBy, string $referenceRoot): array
{
    $projectKey = APP_SAMPLE8_DBACCESS_JOIN_READ_MODEL_PROJECT_KEY;
    $tableNames = APP_SAMPLE8_DBACCESS_JOIN_READ_MODEL_TABLE_NAMES;
    $sourceName = APP_SAMPLE8_DBACCESS_JOIN_READ_MODEL_SOURCE_NAME;
    $functionName = APP_SAMPLE8_DBACCESS_JOIN_READ_MODEL_FUNCTION_NAME;
    $steps = [
        'table_imports' => [],
    ...
    app_sample8_dbaccess_join_read_model_assert_same(3, $dataClassPreview['summary']['table_count'], 'data_class table_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(3, $dataClassPreview['summary']['canonical_data_class_count'], 'data_class canonical_data_class_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(0, $dataClassPreview['summary']['class_insert_count'], 'data_class class_insert_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(0, $dataClassPreview['summary']['class_update_count'], 'data_class class_update_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(0, $dataClassPreview['summary']['field_insert_count'], 'data_class field_insert_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(0, $dataClassPreview['summary']['field_update_count'], 'data_class field_update_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(0, $dataClassPreview['summary']['stale_class_count'], 'data_class stale_class_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(0, $dataClassPreview['summary']['stale_field_count'], 'data_class stale_field_count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(
        $tableNames,
        app_sample8_dbaccess_join_read_model_extract_names($dataClassPreview['classes'], 'name'),
    ...
    app_sample8_dbaccess_join_read_model_assert_same(1, count($classCatalogResult['items']), 'db_access class count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(
        [$sourceName],
        app_sample8_dbaccess_join_read_model_extract_names($classCatalogResult['items'], 'source_name'),
        'db_access class names',
        $assertionErrors,
    );
    app_sample8_dbaccess_join_read_model_assert_same('manual', $classCatalogResult['items'][0]['source_of_truth'] ?? '', 'db_access class source_of_truth', $assertionErrors);
    ...
    app_sample8_dbaccess_join_read_model_assert_same(1, count($functionCatalogResult['items']), 'db_access function count', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(
        [$functionName],
        app_sample8_dbaccess_join_read_model_extract_names($functionCatalogResult['items'], 'function_name'),
    ...
    app_sample8_dbaccess_join_read_model_assert_same('SELECTLIST', $functionResult['item']['action_type'] ?? '', 'db_access function action_type', $assertionErrors);
    app_sample8_dbaccess_join_read_model_assert_same(
        APP_SAMPLE8_DBACCESS_JOIN_READ_MODEL_DATA_CLASS_BASE_NAME,
    ...
    app_sample8_dbaccess_join_read_model_assert_same('manual', $functionResult['item']['source_of_truth'] ?? '', 'db_access function source_of_truth', $assertionErrors);
    ...
    app_sample8_dbaccess_join_read_model_assert_same(
        APP_SAMPLE8_DBACCESS_JOIN_READ_MODEL_SELECT_TARGET_LABELS,
        app_sample8_dbaccess_join_read_model_select_target_labels($selectTargetFieldResult['items']),
    ...
    app_sample8_dbaccess_join_read_model_assert_same('30', $selectWhereResult['items'][2]['where_order'] ?? '', 'db_access fixed author where order', $assertionErrors);

    foreach (APP_SAMPLE8_DBACCESS_JOIN_READ_MODEL_REFERENCE_SOURCE_OUTPUT_KEYS as $sourceOutputKey) {
        ...
        $expectedSnapshot = app_sample8_dbaccess_join_read_model_tree_snapshot($expectedRoot);
        $actualSnapshot = app_sample8_dbaccess_join_read_model_tree_snapshot($publishedRoot);
        ...
        $fileChecks = app_sample8_dbaccess_join_read_model_compare_file_sets(
    ...
    return [
        ...
        'steps' => $steps,
        'assertion_errors' => $assertionErrors,
        'error' => $assertionErrors === [] ? '' : 'sample08 dbaccess join read model assertions failed',
    ];
}
