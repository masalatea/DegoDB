...
`sample01`〜`sample32` は runtime pack であり、`compose.yaml` / `run.sh` / `seed/` を持つ。`sample33` は SQLite-to-MySQL promotion の artifact-chain tutorial であり、first slice では `reference/` fixture と PHPUnit contract で検証する。
