...
import { MazeRoomStore, normalizeRoomSlug } from './maze-room-store.mjs';
...
function createDefaultStore({ dataDir }) {
  return new MazeRoomStore({ filePath: path.join(dataDir, 'maze-room-store.json') });
}
...
function createServer({ store, tickMs = null } = {}) {
  ...
  function subscribe(roomSlug, response) {
    ...
    return () => {
      ...
      if (subscribers.size === 0) subscribersByRoom.delete(roomSlug);
    };
  }
  ...
  function publish(roomSlug, state) {
    ...
    const event = `event: maze.updated\ndata: ${JSON.stringify({ room_slug: roomSlug, revision: state.revision })}\n\n`;
    for (const response of subscribers) response.write(event);
  }
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      if (request.method === 'GET' && url.pathname.startsWith('/r/')) {
        ...
        const roomScript = `<script>globalThis.SAMPLE49_ROOM_SLUG = ${JSON.stringify(roomSlug)};</script>`;
      ...
      if (request.method === 'POST' && joinMatch) {
        ...
        const result = store.joinRoom(roomSlug, { playerId: body.player_id, name: body.name });
        if (result.ok) publish(roomSlug, result.state);
      ...
      if (request.method === 'POST' && commandMatch) {
        ...
        const body = await readJson(request);
        const result = store.applyCommand({ roomSlug, playerId: body.player_id, command: body.command });
        if (result.ok) publish(roomSlug, result.state);
        sendJson(response, result.ok ? 200 : 400, result);
        return;
      }

      const resetMatch = url.pathname.match(/^\/api\/rooms\/([^/]+)\/reset$/);
      if (request.method === 'POST' && resetMatch) {
        const roomSlug = normalizeRoomSlug(decodeURIComponent(resetMatch[1]));
        const result = store.resetRoom(roomSlug);
  ...
  if (tickMs !== null) {
    const timer = setInterval(() => {
      for (const update of store.advanceActiveRooms()) publish(update.room_slug, update.state);
    }, tickMs);
    timer.unref?.();
...
function startServer({
  ...
  host = '127.0.0.1',
  dataDir = process.env.SAMPLE49_DATA_DIR ?? path.join(os.tmpdir(), 'sample49-ai-plugin-maze-race')
} = {}) {
  const store = createDefaultStore({ dataDir });
  const server = createServer({ store, tickMs: 80 });
  server.listen(port, host, () => {
    console.log(`sample49 listening on http://${host}:${port}/r/general`);
