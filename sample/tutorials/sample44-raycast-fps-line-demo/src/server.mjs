...
import { RaycastFpsStore, normalizeRoomSlug } from './raycast-fps-store.mjs';
...
function createDefaultStore({ dataDir }) {
  return new RaycastFpsStore({
    filePath: path.join(dataDir, 'raycast-fps-store.json')
  });
}
  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      if (request.method === 'GET' && url.pathname.startsWith('/r/')) {
        ...
        const roomScript = `<script>globalThis.SAMPLE44_ROOM_SLUG = ${JSON.stringify(roomSlug)};</script>`;
...
function startServer({
  ...
  host = '127.0.0.1',
  dataDir = process.env.SAMPLE44_DATA_DIR ?? path.join(os.tmpdir(), 'sample44-raycast-fps-line-demo')
} = {}) {
  const store = createDefaultStore({ dataDir });
  const server = createServer({ store });
  server.listen(port, host, () => {
    console.log(`sample44 listening on http://${host}:${port}/r/general`);
