...
import { ShooterRoomStore, normalizeRoomSlug } from './shooter-room-store.mjs';
...
function createDefaultStore({ dataDir }) {
  return new ShooterRoomStore({
    filePath: path.join(dataDir, 'shooter-room-store.json')
  });
}
  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      if (request.method === 'GET' && url.pathname.startsWith('/r/')) {
        ...
        const roomScript = `<script>globalThis.SAMPLE42_ROOM_SLUG = ${JSON.stringify(roomSlug)};</script>`;
      ...
      if (request.method === 'POST' && joinMatch) {
        ...
        const result = store.joinRoom(roomSlug);
        if (result.ok) {
...
function startServer({
  ...
  host = '127.0.0.1',
  dataDir = process.env.SAMPLE42_DATA_DIR ?? path.join(os.tmpdir(), 'sample42-room-shooter-game')
} = {}) {
  const store = createDefaultStore({ dataDir });
  const server = createServer({ store });
  server.listen(port, host, () => {
    console.log(`sample42 listening on http://${host}:${port}/r/general`);
