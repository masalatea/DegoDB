...
final class Sample09DbAccessAggregateReportOutputTest extends TestCase
{
    public function testReferenceOutputsStayInSync(): void
    {
        ...
        try {
            $result = app_sample9_dbaccess_aggregate_report_run(
                app_bootstrap(),
                'phpunit',
                app_sample9_dbaccess_aggregate_report_default_reference_root(),
            );
        } finally {
        ...
        self::assertCount(
            count(APP_SAMPLE9_DBACCESS_AGGREGATE_REPORT_REFERENCE_SOURCE_OUTPUT_KEYS),
            $result['steps']['outputs'],
            'unexpected source output count in verification result',
        );

        self::assertCount(
            count(APP_SAMPLE9_DBACCESS_AGGREGATE_REPORT_TABLE_NAMES),
        ...
        self::assertCount(
            count(APP_SAMPLE9_DBACCESS_AGGREGATE_REPORT_SELECT_TARGET_LABELS),
        ...
        self::assertCount(
            2,
            is_array($result['steps']['db_access_select_havings']['items'] ?? null)
                ? $result['steps']['db_access_select_havings']['items']
                : [],
            'unexpected DB Access select having count in verification result',
        );
    ...
    private function failureMessageFromResult(array $result): string
    {
        $payload = [
            ...
            'db_access_select_havings' => $result['steps']['db_access_select_havings'] ?? null,
            'outputs' => array_map(
        ...
        return is_string($encoded) && $encoded !== ''
            ? $encoded
            : 'sample09 dbaccess aggregate report verification returned ok=false';
    }
}
