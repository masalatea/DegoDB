...
Mtool self no-code role boundary:

- not a full replacement target;
- Mtool reaches 100% for this role when it can emit the supported no-code output, contracts, metadata, and handoff artifacts inside its responsibility boundary;
- React/app shell and external framework implementation are separate owner responsibilities;
- any future contained partial/hybrid workflow is new scoped work, not a parked requirement for standalone no-code completion.
