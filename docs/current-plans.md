...
Current main status: #849-#862E completed the SSO app-user standard, policy/bundle, canonical key/FK lifecycle, qualified MySQL/MariaDB live import, and fail-closed generation-readiness gate. The next concrete step is #863: generate the first server runtime slice only from a `generation_ready` canonical fixture. PostgreSQL and SQLite live constraint readers remain explicit cross-driver follow-up scope. / 現在の主計画ステータス: #849〜#862EでSSO app-user標準、policy・bundle、canonical key・FK lifecycle、認定MySQL・MariaDB live import、fail-closed generation-readiness gateを完了しました。次の具体stepは#863で、`generation_ready`のcanonical fixtureだけから最初のserver runtime sliceを生成します。PostgreSQL・SQLiteのlive constraint readerは明示的なcross-driver後続範囲です。
...
| 864 | SSO app-user client/App-local handoff / SSO app-user client・App-local handoff | Carry server-resolved `app_user_id` into the safe App-local/sync actor contract without treating it as client authority | `ACTIVE_NEXT` | safe cache/sync handoff and server revalidation evidence / safe cache・sync handoff・server再検証evidence |
| 865 | SSO app-user cross-driver qualification and checkpoint / SSO app-user cross-driver認定・checkpoint | Record declared driver tests, docs, supported boundary, integration state, and demand-driven remaining work | `PLANNED` | supported generated path is qualified and ready for integration / supported生成path認定・integration準備完了 |
