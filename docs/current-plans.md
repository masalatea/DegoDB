...
Current main status: #410 records the pre-push local stack review after runtime-data product docs refresh. The branch is 92 commits ahead of `origin/develop`; the current reviewable group spans #302-#409 and covers the runtime-data behavior lane plus operator/permanent docs alignment. The stack is suitable for a later push as-is unless the user explicitly asks for history cleanup. No history rewrite or push was performed. / 現在の主計画ステータス: #410 で runtime-data product docs refresh 後の push 前 local stack review を記録しました。branch は `origin/develop` より 92 commits ahead です。現在の reviewable group は #302-#409 にまたがり、runtime-data behavior lane と operator / permanent docs alignment を含みます。user が明示的に history cleanup を求めない限り、stack は後続 push にこのまま載せられます。history rewrite と push は行っていません。
