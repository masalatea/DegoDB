...
- 次は、shared-state chat sample の image attachment metadata / ephemeral store slice を確認し、次の runtime/domain slice またはPR checkpointを選ぶ。
...
- Next is reviewing image attachment metadata / ephemeral store support in the shared-state chat sample and selecting the next runtime/domain slice or PR checkpoint.
...
| 957 | Shared-state sync runtime/domain next slice selection / shared-state sync runtime・domain next slice selection | Decide whether the next slice should be real WebSocket transport, production-hardening checklist, Mtool combined bundle, chat HTTP/SSE route, or checkpoint/PR | `ACTIVE_NEXT` | Choose the next scope after reviewing sample38 runtime slices and sample39 chat text/image-domain slices |
