...
Current main status: #431 chose the first small Mtool no-code dogfooding probe. The probe should start from the Source Output / Shared Contracts / interface profile review surface, because it is already data-flow-first, mostly read/review oriented, and directly exercises the usage intent, view preference, and related-settings navigation layer from #430. Push has not been performed for #431. / 現在の主計画ステータス: #431 で最初の小さな Mtool no-code dogfooding probe を選びました。probe は Source Output / Shared Contracts / interface profile review surface から始めます。ここは既に data-flow-first で、主に read / review 寄りであり、#430 の usage intent、view preference、related-settings navigation layer を直接検証できるためです。#431 は push していません。
...
| A7 | Mtool self no-code replacement / Mtool 自身の no-code 置き換え | Treat as a final dogfooding/replacement program, not a near-term rewrite. #431 promotes the first small dogfooding probe into the mainline: start with the Source Output / Shared Contracts / interface profile review surface, then only later consider broader admin/lab replacement. Small probes are valuable because they exercise generated UI, reveal missing settings/navigation, and validate the database-first two-layer model | `PARKED_WITH_ACTIVE_PROBE` | Large program. The active first probe is small, likely 1 - 2 days through closure; staged replacement remains multiple weeks to months / 大型計画。active first probe は小さく、closure まで 1 - 2 日程度。段階的置き換え全体は引き続き数週間から数か月規模 |
