...
| --- | --- | --- | --- |
| 1 | Post-sample29 no-code product goal replan / sample29 後の no-code product goal 再計画 | Decide the next no-code product-facing slice after the second data-first domain sample: relation/read-model follow-up, app-local sync demo, operator workflow, or runtime polish exposed by sample29 | `ACTIVE_NEXT` |
...
The second data-first no-code domain sample first slice is complete as `sample29-no-code-support-case-demo`. The next active item is a small replan to choose the next product-facing no-code slice from the gaps and opportunities exposed by sample29. / 2 つ目の data-first no-code domain sample first slice は `sample29-no-code-support-case-demo` として完了です。次の active item は、sample29 で見えた gap / opportunity から次の product-facing no-code slice を選ぶ小さな再計画です。
...
| 24 | Post-sample29 no-code product goal replan / sample29 後の no-code product goal 再計画 | 0.5 day / 半日 | Active docs/planning step. Decide whether the next product-facing slice should follow sample29 domain pressure, app-local sync, operator workflow, or targeted runtime polish. |
...
This is the current active planning item after `sample29-no-code-support-case-demo`. / これは `sample29-no-code-support-case-demo` 後の現在の active planning item です。

| Candidate / 候補 | Why / 目的 | First slice estimate / first slice 目安 | Notes |
| --- | --- | --- | --- |
| Sample29 follow-up domain pressure | Use support-case results to add the smallest missing read-model or relation-shaped proof. | 1 - 3 days / 1 - 3 日 | Choose only if sample29 exposes a concrete gap in generated runtime or metadata. |
| App-local sync demonstration | Connect no-code action intent more visibly to App-local persistence / sync concepts. | 2 - 5 days / 2 - 5 日 | Strong next product story, but wider than another sample-only proof. |
...
| Targeted runtime polish from sample29 | Polish only presentation gaps found in the second domain sample. | 0.5 - 2 days / 半日 - 2 日 | Keep this narrow and evidence-driven. |
