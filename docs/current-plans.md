...
Current main status: #430 closed the current no-code usage/view/settings layer. Accepted capability now includes derived and explicit usage intent, persistent view preference, operator/admin interface profiles, Shared Contracts edit UI, and related settings navigation. The next natural step is either local commit cleanup before push, or a small Mtool no-code dogfooding probe as the next implementation lane. / 現在の主計画ステータス: #430 で現在の no-code usage / view / settings layer を closure しました。accepted capability は derived / explicit usage intent、persistent view preference、operator / admin interface profiles、Shared Contracts edit UI、related settings navigation まで含みます。次の自然な作業は push 前の local commit cleanup、または次 implementation lane として小さな Mtool no-code dogfooding probe です。
...
| A7 | Mtool self no-code replacement / Mtool 自身の no-code 置き換え | Treat as a final dogfooding/replacement program, not a near-term rewrite. Small Mtool no-code probes are valuable before that full replacement because they exercise generated UI, reveal missing settings/navigation, and validate the database-first two-layer model. First inventory Mtool admin/lab/source-output screens, then replace low-risk read/review surfaces with generated no-code variants before considering edit/build flows | `PARKED_REPLAN` | Large program. First inventory likely 3 - 5 days; staged replacement is likely multiple weeks to months / 大型計画。最初の棚卸しは 3 - 5 日程度、段階的置き換えは数週間から数か月規模 |
