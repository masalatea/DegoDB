...
Current main status: #553 adds `sample32-no-code-ui-test-lab`, a small dedicated no-code fixture sample with list/detail/form metadata, a disabled managed action, fixed preview rows, and fast JSON/DOM contract tests. `develop` is 29 commits ahead of `origin/develop`, and push has not been performed for #432-#553. / 現在の主計画ステータス: #553 で `sample32-no-code-ui-test-lab` を追加しました。list/detail/form metadata、disabled managed action、fixed preview rows、fast JSON/DOM contract tests を持つ小さな No Code 専用 fixture sample です。`develop` は `origin/develop` より 29 commits ahead、#432-#553 は push していません。
...
| 554 | No-code sample contract fixture ladder / no-code sample contract fixture ladder | Grow the dedicated sample through small fixtures, each with metadata JSON and DOM contract assertions before browser smoke | `ACTIVE_NEXT` | 1 - 2 days / 1 - 2 日 |
