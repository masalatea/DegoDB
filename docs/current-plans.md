...
| --- | --- | --- | --- |
| 1 | App-local persistence first demo / App-local persistence first demo | App-local SQLite schema, App-local DBAccess, and sample27 DTO round trip | `ACTIVE_NEXT` |
| 2 | Managed data operation layer / managed data operation layer | Operation metadata, permission policy binding, and sync skeleton | `TODO` |
| 3 | No-code screen definition and runtime MVP / no-code screen definition・runtime MVP | Generated list/detail/form definitions and minimal no-code runtime | `TODO` |
| 4 | No-code app sample / no-code app sample | User-facing data-first no-code sample that proves the roadmap end to end | `TODO` |
| 5 | Mtool implementation namespace cleanup / Mtool 実装 namespace cleanup | Separate cleanup commit after generated output namespace support settles | `LATER` |
