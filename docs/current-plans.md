...
Current main status: #794 closes the sample capability coverage lane. The required no-code capability matrix now has no remaining GAP entries: supported contracts are covered by representative sample evidence, and hard-delete / file-upload / rich-editor / full-sample conversion are explicitly not required for the initial matrix. The next active lane is #795: select one contained Mtool hybrid replacement workflow without starting a broad rewrite. / 現在の主計画ステータス: #794でsample capability coverage laneを閉じました。必須No Code capability matrixに残るGAPはありません。support対象contractは代表sample evidenceでcoveredとなり、hard delete・file upload・rich editor・全sample完全変換はinitial matrixでは明示not requiredです。次のactive laneは#795で、広範rewriteではなく、閉じたMtool hybrid replacement workflowを1つ選定します。
...
| 795 | Contained Mtool hybrid replacement selection / Mtool contained hybrid置換候補選定 | Choose one low-risk Mtool workflow and define generated/custom/integration responsibilities before implementation | `ACTIVE_NEXT` | 0.5 - 1 day / 半日 - 1 日 |
...
| N5 | Contained hybrid Mtool no-code replan / Mtool部分hybrid No Code再計画 | Select one Mtool workflow and divide generated/no-code, custom, and integration responsibilities | N4 closes the agreed capability matrix | One partial replacement slice has scope, coexistence boundary, rollback, authority, and tests | `ACTIVE_NEXT` |
| N6 | AI material-to-UI replan / AI資料to UI再計画 | Select source material, Q&A purpose, normalized structure, and generated UI/action target | Mtool self-no-code phase provides a credible reusable product surface | One bounded G-L5 end-to-end investigation is selected | `LONG_TERM_PARKED` |

The binding order is N3/N4 capability coverage through samples, then N5 contained hybrid Mtool replacement, then N6 AI material-to-UI. This is capability-driven rather than sample-count-driven: neither every sample nor every Mtool screen must be fully replaced. A feasibility probe in a later phase does not mark the earlier capability matrix complete and does not authorize broad rollout. / 拘束的な順序はN3/N4 sampleによるcapability網羅、次にN5 contained hybrid Mtool部分置換、その後N6 AI資料to UIです。sample数ではなくcapability基準であり、全sample・全Mtool screenの完全置換は要求しません。後段phaseのfeasibility probeは前段capability matrixの完了を意味せず、広範展開も許可しません。
...
| L2 | Hybrid Mtool self no-code / Mtool 自身のhybrid No Code化 | Partially replace contained Mtool workflows after capability coverage, explicitly mixing generated/no-code and custom portions. / capability網羅後、generated・No Code部分とcustom部分を明示的に混在させて閉じたMtool workflowを部分置換する。 | `ACTIVE_NEXT` |
...
Gate status: G-L1 through G-L4 have bounded feasibility evidence, but product work remains in L1 capability coverage. Sample18 and Sample29 provide representative evidence; completion depends on the agreed capability matrix, not conversion of all samples. The Mtool inspection proves a contained read-only probe only; later Mtool work may be partial and hybrid rather than a full replacement. The task-packet/schema-review workflow proves the G-L4 review boundary only, not material-to-UI productization. G-L5 remains long-term. / Gate status: G-L1〜G-L4にはboundedなfeasibility evidenceがありますが、product作業はL1 capability網羅にあります。Sample18・Sample29は代表evidenceであり、完了は全sample変換ではなく合意capability matrixで判定します。Mtool inspectionは限定read-only probeだけを実証し、後続Mtool作業は全面置換ではなく部分的hybridで構いません。task packet・schema reviewはG-L4 review境界だけを実証し、資料to UIのproduct化を意味しません。G-L5は長期目標です。
