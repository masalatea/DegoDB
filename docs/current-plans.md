...
| --- | --- | --- | --- |
| 1 | Shared Contract Core Vocabulary / shared contract 最小語彙 | Contract manifest v0 shape, validator, and sample02/task fixture test | `ACTIVE_NEXT` |
| 2 | Shared DataClass contract foundation / shared DataClass contract 基盤 | Contract metadata, contract manifest generator, TypeScript DTO first output, and contract compare | `TODO` |
...
App local DB / sync / no-code app roadmap and feasibility study catalog are drafted as dated reports. During planning, auth foundation was identified as useful beyond that roadmap, so it was promoted out of the FS group into a normal first-slice feature/foundation plan. Mtool auth foundation first slice is complete. Gate 0 core feasibility studies are also complete: Shared Contract Manifest Spike, App Local SQLite Schema Spike, and DTO Save/Read Spike. The FS result supports the original design assumption that generated DataClass and shared contract should be separate artifacts: DataClass remains implementation-facing, while shared contract metadata carries persistence / sync / no-code semantics. Before starting the larger Shared DataClass contract foundation, the active next work is Shared Contract Core Vocabulary: define contract manifest v0 shape, validator, and sample02/task fixture coverage. After that, the roadmap proceeds through shared DataClass contract foundation, App-local persistence, managed data operations, no-code screen/runtime MVP, and a no-code app sample. / App 内 DB・同期・no-code app roadmap と feasibility study catalog は日付付き report として作成済みです。検討の中で auth 基盤はその roadmap に限定されず通常機能としても有用だと分かったため、FS 群から外して正式な first slice 計画へ格上げしました。Mtool auth 基盤 first slice は完了済みです。Gate 0 core FS も Shared Contract Manifest Spike、App Local SQLite Schema Spike、DTO Save/Read Spike まで完了し、generated DataClass と shared contract は別 artifact として扱うべき、という元の見立てを支持しました。DataClass は implementation-facing、shared contract metadata は persistence / sync / no-code semantics の正本として扱います。大きめの Shared DataClass contract 基盤へ入る前に、次の active work は Shared Contract Core Vocabulary とし、contract manifest v0 shape、validator、sample02/task fixture coverage を固定します。その後、roadmap は shared DataClass contract 基盤、App-local persistence、managed data operation、no-code screen/runtime MVP、no-code app sample へ進みます。
...
| 11 | Shared Contract Core Vocabulary / shared contract 最小語彙 | 0.5-1.5 days / 0.5-1.5 日 | Add the shared contract manifest v0 vocabulary and validator before generator work: contract key, entity identity, field identity, type, nullable, default, key, storage/local metadata collision policy, manifest version, and sample02/task fixture test. |
| 12 | Shared DataClass contract foundation / shared DataClass contract 基盤 | 1-2 weeks / 1-2 週間 | Build on the core vocabulary. Add separate contract metadata, language-neutral contract manifest generator, TypeScript DTO first output, and contract compare. |
| 13 | App-local persistence first demo / App-local persistence first demo | 2-3 weeks / 2-3 週間 | App-local SQLite schema, App-local DBAccess, Node harness, and sample27 `server read -> DTO -> app save -> app read`. |
| 14 | Managed data operation layer / managed data operation layer | 2-4 weeks / 2-4 週間 | Operation definitions, operation fields, permission policy binding, storage-role checks, and sync skeleton. |
| 15 | No-code screen definition and runtime MVP / no-code screen definition・runtime MVP | 5-9 weeks / 5-9 週間 | Generated list/detail/form definitions first, then minimal runtime that renders definitions and executes allowed operations. |
| 16 | No-code app sample / no-code app sample | 1-3 weeks after runtime MVP / runtime MVP 後 1-3 週間 | User-facing sample proving data-first no-code behavior end to end. Candidate: `sample28-no-code-data-app-mvp`. |
| 17 | Mtool implementation namespace cleanup / Mtool 実装 namespace cleanup | 30-90 min for first mechanical cleanup; half-day+ if runtime/autoload/tests broaden / 最初の機械的 cleanup は 30-90 分。runtime / autoload / tests まで広げるなら半日以上 | Deferred until generated output namespace support settles. Start by classifying `rg` results and keep docs/comment/generated-reference wording separate from runtime class or autoload changes. |
