...
| --- | --- | --- | --- |
| 1 | App-local sync no-code demonstration first slice / App-local sync no-code demonstration first slice | Connect a generated no-code action intent to the existing App-local persistence / managed operation sync foundations through a small, sample-bound proof | `ACTIVE_NEXT` |
...
The second data-first no-code domain sample first slice is complete as `sample29-no-code-support-case-demo`, and the post-sample29 replan selected an App-local sync no-code demonstration as the next product-facing slice. / 2 つ目の data-first no-code domain sample first slice は `sample29-no-code-support-case-demo` として完了し、sample29 後の replan では App-local sync no-code demonstration を次の product-facing slice として選びました。
...
| 26 | Post-sample30 no-code product goal replan / sample30 後の no-code product goal 再計画 | 0.5 day / 半日 | Active docs/planning step. Decide the next product-facing slice after the first sync-backed no-code demonstration. |
...
This is the current active planning item after `sample30-no-code-app-local-sync-demo`. / これは `sample30-no-code-app-local-sync-demo` 後の現在の active planning item です。

| Candidate / 候補 | Why / 目的 | First slice estimate / first slice 目安 | Notes |
| --- | --- | --- | --- |
| Sync handoff visibility polish | Make the generated/runtime artifact show the sync handoff state more clearly. | 0.5 - 2 days / 半日 - 2 日 | Choose only if the sample30 result exposes a concrete presentation gap. |
| Server-side sync processing follow-up | Extend the sample-visible path from App-local outbox handling toward server DBAccess processing. | 1 - 3 days / 1 - 3 日 | Keep conflict resolution and transport out of scope. |
| Operator/admin no-code workflow | Show how an operator chooses, publishes, or inspects no-code runtime artifacts. | 1 - 3 days / 1 - 3 日 | Needs a clearer operator surface before implementation. |
| Mtool implementation namespace cleanup | Revisit the parked namespace cleanup with a concrete helper cluster. | 1 - 3 days / 1 - 3 日 | Only reopen if a narrow helper cluster is selected. |
