...
| --- | --- | --- | --- |
| 1 | Gate 0: App local DB / sync / no-code roadmap feasibility studies / Gate 0: App 内 DB・同期・no-code roadmap の FS 群 | Validate data-contract / App-local feasibility after auth foundation; detailed FS catalog is tracked separately / auth 基盤後に data contract・App-local の feasibility を確認する。FS 詳細は別 report を参照 | `ACTIVE_NEXT` |
| 2 | Shared DataClass contract foundation / shared DataClass contract 基盤 | Contract metadata, contract manifest, and TypeScript DTO first output | `TODO` |
| 3 | App-local persistence first demo / App-local persistence first demo | App-local SQLite schema, App-local DBAccess, and sample27 DTO round trip | `TODO` |
| 4 | Managed data operation layer / managed data operation layer | Operation metadata, permission policy binding, and sync skeleton | `TODO` |
| 5 | No-code screen definition and runtime MVP / no-code screen definition・runtime MVP | Generated list/detail/form definitions and minimal no-code runtime | `TODO` |
| 6 | No-code app sample / no-code app sample | User-facing data-first no-code sample that proves the roadmap end to end | `TODO` |
| 7 | Mtool implementation namespace cleanup / Mtool 実装 namespace cleanup | Separate cleanup commit after generated output namespace support settles | `LATER` |
...
App local DB / sync / no-code app roadmap and feasibility study catalog are drafted as dated reports. During planning, auth foundation was identified as useful beyond that roadmap, so it was promoted out of the FS group into a normal first-slice feature/foundation plan. Mtool auth foundation first slice is now complete: legacy permission units are inventoried, collapsed into role-based permission keys, normalized principal shape is defined, and a minimal all-pass / fail-closed authorization evaluator is covered by contract tests. The active next work is Gate 0 feasibility studies, starting with Shared Contract Manifest Spike or App Local SQLite Schema Spike. After that, the roadmap proceeds through shared DataClass contract, App-local persistence, managed data operations, no-code screen/runtime MVP, and a no-code app sample. / App 内 DB・同期・no-code app roadmap と feasibility study catalog は日付付き report として作成済みです。検討の中で auth 基盤はその roadmap に限定されず通常機能としても有用だと分かったため、FS 群から外して正式な first slice 計画へ格上げしました。Mtool auth 基盤 first slice は完了し、旧 permission unit の棚卸し、role-based permission key への整理、normalized principal shape、最小 all-pass / fail-closed authorization evaluator を contract test で固定済みです。次の active work は Gate 0 FS で、Shared Contract Manifest Spike または App Local SQLite Schema Spike から始めます。その後、roadmap は shared DataClass contract、App-local persistence、managed data operation、no-code screen/runtime MVP、no-code app sample へ進みます。
...
| 10 | Gate 0 feasibility studies / Gate 0 FS 群 | 1-3 days for data-contract/App-local first studies / data contract・App-local の first studies は 1-3 日 | Details are in [2026-0628 App Local DB Feasibility Studies](reports/2026/2026-0628-app-local-db-feasibility-studies.md). Gate 0 is not no-code implementation. Run after auth foundation first slice, starting with Shared Contract Manifest Spike or App Local SQLite Schema Spike. |
| 11 | Shared DataClass contract foundation / shared DataClass contract 基盤 | 1-2 weeks / 1-2 週間 | Promote after Gate 0 passes. Add separate contract metadata, language-neutral contract manifest, TypeScript DTO first output, and contract compare. |
| 12 | App-local persistence first demo / App-local persistence first demo | 2-3 weeks / 2-3 週間 | App-local SQLite schema, App-local DBAccess, Node harness, and sample27 `server read -> DTO -> app save -> app read`. |
| 13 | Managed data operation layer / managed data operation layer | 2-4 weeks / 2-4 週間 | Operation definitions, operation fields, permission policy binding, storage-role checks, and sync skeleton. |
| 14 | No-code screen definition and runtime MVP / no-code screen definition・runtime MVP | 5-9 weeks / 5-9 週間 | Generated list/detail/form definitions first, then minimal runtime that renders definitions and executes allowed operations. |
| 15 | No-code app sample / no-code app sample | 1-3 weeks after runtime MVP / runtime MVP 後 1-3 週間 | User-facing sample proving data-first no-code behavior end to end. Candidate: `sample28-no-code-data-app-mvp`. |
| 16 | Mtool implementation namespace cleanup / Mtool 実装 namespace cleanup | 30-90 min for first mechanical cleanup; half-day+ if runtime/autoload/tests broaden / 最初の機械的 cleanup は 30-90 分。runtime / autoload / tests まで広げるなら半日以上 | Deferred until generated output namespace support settles. Start by classifying `rg` results and keep docs/comment/generated-reference wording separate from runtime class or autoload changes. |
...
| App local DB / sync roadmap and feasibility catalog / App 内 DB・同期 roadmap と FS catalog | [2026-0628 App Local DB And Sync Roadmap](reports/2026/2026-0628-app-local-db-and-sync-roadmap.md), [2026-0628 App Local DB Feasibility Studies](reports/2026/2026-0628-app-local-db-feasibility-studies.md). Roadmap / FS catalog drafting is done; individual FS studies are not yet executed. |
