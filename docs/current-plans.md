...
| --- | --- | --- | --- |
| 1 | No-code screen definition and runtime MVP / no-code screen definition・runtime MVP | Screen definition, runtime adapter, runtime JSON/HTML artifact path, and persisted operation flow are in place; basic UI smoke next | `ACTIVE_NEXT` |
| 2 | No-code app sample / no-code app sample | User-facing data-first no-code sample that proves the roadmap end to end | `TODO` |
| 3 | Mtool implementation namespace cleanup / Mtool 実装 namespace cleanup | Separate cleanup commit after generated output namespace support settles | `LATER` |
...
| 16 | No-code screen definition and runtime MVP / no-code screen definition・runtime MVP | 5-9 weeks / 5-9 週間 | Active. `no-code-screen-definition-v0`, first `no-code-runtime-v0` render/dispatch adapter, `no-code-runtime-json` artifact publishing with HTML preview, sample07 artifact generation/publish verification, and a persisted operation flow are in place; next slice should add basic UI smoke. |
| 17 | No-code app sample / no-code app sample | 1-3 weeks after runtime MVP / runtime MVP 後 1-3 週間 | User-facing sample proving data-first no-code behavior end to end. Candidate: `sample28-no-code-data-app-mvp`. |
...
## No-Code Completion Breakdown / no-code 完成までの分解

This section expands Quick Plan item 1 and item 2 into implementation-sized steps. / この section は Quick Plan の 1 と 2 を、実装単位へ展開します。
...
| 7 | Basic UI smoke for list/detail/form / list/detail/form basic UI smoke | `ACTIVE_NEXT` | 1 - 2 days / 1 - 2 日 | Add browser/headless smoke for generated HTML preview. |
| 8 | Create/update browser or headless smoke / create/update browser または headless smoke | `TODO` | 1 - 3 days / 1 - 3 日 | Prove action dispatch goes through generated operation boundary, not hand-coded screen logic. |
| 9 | `sample28-no-code-data-app-mvp` / `sample28-no-code-data-app-mvp` | `TODO` | 3 days - 1.5 weeks / 3 日 - 1.5 週間 | User-facing sample proving the data-first no-code path end to end. |
| 10 | No-code docs, reports, and plan cleanup / no-code docs・reports・plan cleanup | `TODO` | 0.5 - 1 day / 半日 - 1 日 | Move completed items to dated reports and keep Quick Plan short. |

Rough total from the current state / 現在地からの合計目安:

- Minimal no-code runtime MVP means steps 1-8: 1 - 2 weeks. / 最小 no-code runtime MVP は step 1-8 を指し、1 - 2 週間。
