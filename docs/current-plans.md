...
Current main status: #849-#858 completed and merged the first App-local identity and SSO application-user standard/proof lanes. #859-#861 added the DB-first integration plan, project policy contract, and optional metadata bundle round-trip. #862 now validates every SSO app-user invariant expressible in current Table/DataClass/DBAccess metadata, but correctly blocks generation because current canonical metadata cannot prove composite unique keys or database foreign keys. The next concrete step is #862A: preflight a canonical constraint metadata/evidence boundary before the generated server runtime slice. / 現在の主計画ステータス: #849〜#858で最初のApp-local identityおよびSSO application-user標準・proof laneを完了・merge済みです。#859〜#861でDB-first integration plan、project policy contract、optional metadata bundle round-tripを追加しました。#862では現行Table・DataClass・DBAccess metadataで表現可能なSSO app-user invariantを検証しますが、canonical metadataが複合unique keyとdatabase foreign keyを証明できないため、生成readyにはせず正しくblockします。次の具体stepは#862Aで、generated server runtime sliceの前にcanonical constraint metadata・evidence境界をpreflightします。
