...
The physical/logical sample naming migration and PostgreSQL Input / Output support are complete for their intended support boundaries. The next implementation lane should start only when namespace cleanup scope or another parked item is made explicit. / physical / logical sample 命名移行と PostgreSQL Input / Output 対応は、意図した support boundary では完了です。次の implementation lane は、namespace cleanup scope または別の parked item が明示された時だけ開始します。
