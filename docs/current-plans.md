...
The physical/logical sample naming migration is complete for the tracked tutorial samples. The next implementation lane is conditional and should start only when a PostgreSQL follow-up or namespace cleanup scope is made explicit. / physical / logical sample 命名移行は、追跡対象 tutorial sample では完了です。次の implementation lane は条件付きで、PostgreSQL follow-up または namespace cleanup の scope が明示された時だけ開始します。
