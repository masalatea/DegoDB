...
Current main status: #552 adds the first fast PHPUnit JSON and `DOMDocument` no-code UI contract harness and applies it to sample18 generated artifacts without launching headless Chrome. `develop` is 28 commits ahead of `origin/develop`, and push has not been performed for #432-#552. / 現在の主計画ステータス: #552 で最初の fast PHPUnit JSON / `DOMDocument` no-code UI contract harness を追加し、headless Chrome を起動せず sample18 generated artifact に適用しました。`develop` は `origin/develop` より 28 commits ahead、#432-#552 は push していません。
...
| 553 | Dedicated no-code UI test lab sample / dedicated no-code UI test lab sample | Add a small no-code-only sample focused on list/detail/form/disabled action fixtures and fast UI contract tests | `ACTIVE_NEXT` | 1 day / 1 日 |
