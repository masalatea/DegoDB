...
Current main status: #663 defines generated-submit route feature-flag integration and promotes the first route-level execution slice. `develop` is 141 commits ahead of `origin/develop`, and push has not been performed for #432-#663. / 現在の主計画ステータス: #663 で generated-submit route feature-flag integration を定義し、最初の route-level execution slice を昇格しました。`develop` は `origin/develop` より 141 commits ahead、#432-#663 は push していません。
...
| 664 | Sample18 generated submit route feature-flag execution first slice / sample18 generated submit route feature-flag execution first slice | Wire the generated-submit route to execute the route execution plan only when the explicit executor feature flag is enabled, using transaction binding and DB-backed post-commit recorders while preserving disabled default and all-success-or-failure responses | `ACTIVE_NEXT` | 1 - 1.5 days / 1 - 1.5 日 |
