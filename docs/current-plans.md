...
Current main status: #847 closed the AI workspace onboarding guide lane without promoting a speculative adoption workflow. Sample no-code and Mtool self no-code are complete under the supported-contract definition, while AI design assistance is a separate planning layer. The next concrete candidate is #848: review the local commit stack and decide push/PR, hold, or cleanup. / 現在の主計画ステータス: #847でAI workspace onboarding guide laneを閉じ、具体需要のないadoption workflowは昇格しませんでした。sample No CodeとMtool自身No Codeはsupported-contract定義で完了し、AI設計支援は別のplanning layerです。次の具体候補は#848で、local commit stackを確認し、push/PR・hold・cleanupを判断します。
...
| 848 | Post AI workspace onboarding local stack checkpoint / AI workspace onboarding後local stack checkpoint | Inspect branch divergence, clean tree, recent semantic commits, and decide whether to push/open PR, hold locally, or clean the stack further before more product work | `ACTIVE_NEXT` | push/PR・hold・cleanup判断 / push・PR・hold・cleanup判断 |
