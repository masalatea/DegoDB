...
Current main status: #651 defines the real transaction binding boundary and promotes a route-unwired transaction binding helper before route wiring. `develop` is 129 commits ahead of `origin/develop`, and push has not been performed for #432-#651. / 現在の主計画ステータス: #651 で real transaction binding boundary を定義し、route wiring より前に route-unwired transaction binding helper を昇格しました。`develop` は `origin/develop` より 129 commits ahead、#432-#651 は push していません。
