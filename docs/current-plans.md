...
Current main status: #849-#866 completed and merged the first SSO app-user generated integration lane through PR #69. The new main lane is one-way SQLite-first to MySQL/MariaDB promotion: preserve canonical metadata, generate a fresh target schema, copy and validate data during an explicit write freeze, switch connection only after acceptance, and retain SQLite as rollback evidence. MySQL-to-SQLite, bidirectional sync, and zero-downtime CDC are not standard goals. An independent optional-AI fallback lane (#877-#883) generalizes the existing Sample19 deterministic scan and Ollama first slice without displacing Codex/Claude as the primary path. / 現在の主計画ステータス: #849〜#866の最初のSSO app-user生成integration laneはPR #69でmerge済みです。新しい主線はSQLite-firstからMySQL・MariaDBへの一方向promotionです。canonical metadataを維持し、fresh target schemaを生成し、明示write freeze中にdata copy・検証し、accept後だけ接続先を切り替え、SQLiteをrollback根拠として保持します。MySQLからSQLite、双方向sync、zero-downtime CDCは標準目標に含めません。独立したoptional AI fallback lane (#877〜#883)では、Codex・Claudeを主経路のまま維持し、既存Sample19のdeterministic scan・Ollama first sliceを汎用化します。
...
| 852 | Post App-local sync identity stack checkpoint / App-local sync identity後stack checkpoint | Inspect branch divergence, clean tree, semantic commit shape, and decide whether to push/open PR, hold locally, or clean the stack further before broader rollout | `DONE` | PR merged to `develop`; local `develop` fast-forwarded; feature branch cleaned / PRを`develop`へmerge済み・local `develop` fast-forward済み・feature branch整理済み |
| 853 | Next demand-driven product lane inventory / 次の需要駆動product lane棚卸し | Select the SSO application-user standard-direction lane and record its product goal, identity boundary, and implementation sequence | `DONE_LANE_SELECTED` | SSO app-user standard-direction plan recorded / SSO app-user標準方向plan記録済み |
| 854 | SSO app-user existing-contract inventory / SSO app-user既存contract棚卸し | Map OIDC principals, identity membership, App-local identity, generated DBAccess, and existing user-data examples to the selected standard; identify gaps without implementation | `DONE` | reusable contracts, gaps, decisions, and first implementation boundary recorded / 再利用contract・gap・判断・first implementation境界を記録済み |
| 855 | SSO app-user stable design standard / SSO app-user恒久設計標準 | Define the permanent Mtool/AI-facing standard for external identity, stable app user ID, conceptual schema, provisioning, profile ownership, lifecycle, security, and custom boundaries | `DONE` | `docs/sso-application-user-standard.md` linked from docs entrances / 恒久設計書をdocs入口へ接続済み |
| 856 | SSO app-user Mtool design guidance contract / SSO app-user Mtool設計誘導contract | Make an SSO project recognizable to Mtool/AI and produce the standard recommendation plus only material user questions | `DONE` | side-effect-free guidance + fail-closed validator + focused/full tests / side-effect-free誘導・fail-closed validator・focused/full test済み |
| 857 | SSO app-user representative implementation proof / SSO app-user代表実装proof | Prove transactional JIT creation, repeat-login restore, safe profile refresh, and application-owned data keyed by `app_user_id` in one bounded generated/server-client scenario | `DONE_FIRST_SQLITE_PROOF` | JIT/restore/profile/deny/rollback/domain ownership runtime evidence / JIT・復元・profile・拒否・rollback・domain ownership実証済み |
| 858 | SSO app-user lane checkpoint / SSO app-user lane checkpoint | Record test evidence, supported boundary, documentation entrances, integration state, and only concrete remaining demand-driven work | `DONE_MERGED` | PR #68 merged to `develop`; local develop synchronized; feature branch cleaned / PR #68を`develop`へmerge・local同期・feature branch整理済み |
| 859 | SSO app-user generated integration preflight / SSO app-user生成integration事前設計 | Define how project metadata captures provisioning/profile ownership and how generation emits user-DB schema, resolver/runtime handoff, validation, and tests from the proven standard | `DONE` | project policy + canonical schema ownership + optional migration-safe sequence defined / project policy・canonical schema所有・optional migration-safe sequence定義済み |
| 860 | SSO app-user project policy contract / SSO app-user project policy contract | Add a versioned side-effect-free policy normalizer/validator and one-row-per-project repository without implicit user-DB mutation | `DONE` | policy/repository/bootstrap + focused/full tests; no user DB mutation / policy・repository・bootstrap・focused/full test、user DB変更なし |
| 861 | SSO app-user metadata bundle integration / SSO app-user metadata bundle integration | Add optional policy export/import preview/apply while preserving projects and bundles that do not opt in | `DONE` | optional file + create/replace/disable/preserve preview + compatibility tests / optional file・preview semantics・互換test済み |
| 862 | SSO app-user canonical schema role validation / SSO app-user canonical schema role validation | Resolve policy roles to normal Table/DataClass/DBAccess metadata and fail closed on missing identity/schema invariants | `DONE_EXPRESSIBLE_SCOPE_GAP_IDENTIFIED` | table/field/PK/DataClass relation/DBAccess validated; composite unique/FK evidence explicitly blocked / table・field・PK・DataClass relation・DBAccess検証、複合unique・FK evidenceを明示block |
| 862A | Canonical constraint metadata preflight / canonical constraint metadata事前設計 | Decide how composite unique keys and foreign keys become canonical, reviewable, bundle-safe, and cross-driver evidence before runtime generation | `DONE` | normalized key/FK model, driver sources, import/bundle lifecycle, compatibility, and first slice selected / key・FK model・driver source・import/bundle・互換・first slice選定済み |
| 862B | Canonical constraint metadata foundation / canonical constraint metadata基盤 | Add config schema, validator, transactional repository, and focused composite key/FK tests without changing import or generators | `DONE` | ordered composite key/FK repository + strict validation + focused/full tests / ordered複合key・FK repository・厳格validation・focused/full test済み |
| 862C | Canonical constraint metadata bundle integration / canonical constraint metadata bundle integration | Add optional portable key/FK sections without changing legacy bundle output | `DONE` | physical-name portable files + PID rebind + legacy preserve + focused/full tests / portable file・PID再bind・legacy preserve・focused/full test済み |
| 862D | Live schema constraint import first slice / live schema constraint import first slice | Import one driver's keys/FKs through table preview/apply and record remaining driver scope | `DONE_MYSQL_MARIADB` | managed-table-scoped key/FK reader + preview counts + same-transaction apply; PostgreSQL/SQLite explicit follow-up / managed table限定reader・preview件数・同一transaction apply、PostgreSQL・SQLiteは後続 |
| 862E | SSO schema constraint gate completion / SSO schema constraint gate完了 | Consume canonical unique/FK metadata and allow generation-ready only when required constraints are proven | `DONE` | ordered unique + identity/profile FK evidence reaches `generation_ready`; missing/wrong evidence remains blocked / ordered unique・identity/profile FK evidenceでready、不足・誤証拠はblock |
| 863 | SSO app-user first generated server runtime slice / SSO app-user first generated server runtime slice | Generate one server output using generated DBAccess plus composite transaction resolver and canonical app user actor result | `DONE_SQLITE_QUALIFIED` | generated first-login/repeat-login/rollback + real Source Output artifact evidence / 生成login・rollback・real artifact実証 |
| 863A | Generated SSO DBAccess invocation contract preflight / 生成SSO DBAccess呼出contract事前設計 | Fix deterministic operation names, inputs/results, transaction ownership, output artifact, gate, and failure semantics before generation | `DONE` | no guessed action-only dispatch; exact convention selected / action種別だけの推測dispatch禁止・exact convention選定 |
| 863B | Generated SSO resolver contract and gate / 生成SSO resolver contract・gate | Add a post-schema exact resolver-callable DBAccess operation gate and emit the bounded resolver contract artifact only when ready | `DONE` | deterministic operation-map artifact + schema/missing/ambiguous/wrong-action fail-closed boundary / deterministic artifact・fail-closed境界 |
| 863C | Generated SSO resolver DBAccess binding first slice / 生成SSO resolver DBAccess binding first slice | Bind the exact operation map to one generated DBAccess fixture and implement composite resolver execution | `DONE_SQLITE_FIXTURE` | first-login/repeat-login/full-rollback + credential exclusion + canonical actor result / first login・repeat login・full rollback・credential除外・actor result |
| 863D | Source Output SSO resolver artifact integration / Source Output SSO resolver artifact統合 | Add gated resolver contract/executable support to runtime staging without changing disabled/non-SSO outputs | `FIRST_SLICE_DONE` | conditional staging + manifest summary + non-SSO full regression / 条件付き配置・manifest summary・非SSO回帰 |
| 863E | Qualified SSO Source Output fixture / 認定SSO Source Output fixture | Build one enabled generation-ready project fixture through the real runtime preparation path | `DONE` | emitted files/autoload/manifest verified; disabled regeneration removes resolver / file・autoload・manifest実証、disabled再生成で削除 |
| 864 | SSO app-user client/App-local handoff / SSO app-user client・App-local handoff | Carry server-resolved `app_user_id` into the safe App-local/sync actor contract without treating it as client authority | `DONE` | persisted server provenance + correlation-only actor + server match revalidation + stale rejection / server provenance保存・correlation限定・server再検証・stale拒否 |
| 865 | SSO app-user cross-driver qualification and checkpoint / SSO app-user cross-driver認定・checkpoint | Record declared driver tests, docs, supported boundary, integration state, and demand-driven remaining work | `DONE_SQLITE_E2E_MYSQL_PARTIAL` | SQLite E2E supported; MySQL/MariaDB foundation only; PostgreSQL follow-up explicit / SQLite E2E認定・MySQL基盤認定・PostgreSQL後続明示 |
| 866 | SSO app-user generated integration stack checkpoint / SSO app-user生成integration stack checkpoint | Inspect clean tree, semantic commits, divergence, test evidence, and prepare integration/PR decision | `DONE_MERGED` | five semantic commits merged by PR #69; develop synchronized; feature cleaned / 5意味commitをPR #69でmerge・develop同期・feature整理済み |
| 867 | SQLite-to-MySQL promotion inventory and preflight / SQLiteからMySQL promotion棚卸し・事前設計 | Fix supported source/target, downtime, authority, type/value hazards, artifact ownership, rollback, and non-goals before migration code | `DONE` | exact v1 contract, blocker matrix, dedicated fixture shape, reusable foundations, gaps, and first implementation boundary recorded / v1 contract・blocker・専用fixture・再利用基盤・gap・first実装境界を確定 |
| 868 | Promotion manifest and side-effect-free planning contract / promotion manifest・副作用なし計画contract | Produce a reviewable plan from canonical metadata plus SQLite inspection without writing target data | `DONE_FIRST_SLICE` | deterministic v1 manifest builder, type/value blockers, FK load order/cycle gate, constraint evidence, secret redaction, and contract tests / 決定的v1 manifest・型/value blocker・FK順序/cycle gate・constraint evidence・secret除外・contract test済み |
| 869 | Fresh MySQL target schema preparation / fresh MySQL target schema準備 | Generate/verify an empty MySQL target from canonical metadata and reject non-empty or incompatible targets by default | `DONE_MYSQL_MARIADB_LIVE` | deterministic DDL/digest, allowlists, empty-target gate, approval, partial-failure semantics, isolated live apply/reinspection, FK evidence, and non-empty rejection qualified / DDL・digest・allowlist・空target・承認・partial failure・隔離live apply・FK・非empty拒否を認定 |
| 870 | Deterministic SQLite export and value conversion / 決定的SQLite export・値変換 | Stream rows in stable PK order and convert only declared type/value mappings | `DONE_FIRST_SLICE` | SQLite read snapshot, stable PK/keyset resume, bounded callback chunks, source count checks, canonical JSON, BLOB base64, lossless scalar conversion, and chunk digests / read snapshot・PK resume・bounded callback・件数・JSON・BLOB・lossless変換・digest済み |
| 871 | Transactional MySQL import and resume / transactional MySQL import・再開 | Load parent/child tables in planned order with per-chunk atomicity, checkpoints, and safe retry | `DONE_FIRST_SLICE_MYSQL_LIVE` | chunk integrity, insert-only transaction, JSON/BLOB decode, commit-after checkpoint, checkpoint no-op retry, duplicate rollback, and isolated MariaDB proof / chunk検証・insert-only transaction・decode・commit後checkpoint・retry no-op・重複rollback・MariaDB実証 |
| 872 | Promotion verification gate / promotion検証gate | Compare row counts, PKs, selected row digests, nullability, unique/FK integrity, sequences, JSON/BLOB, and timestamps | `DONE_V1_FULL_GATE` | all required checks now assemble from collectors plus versioned DBAccess smoke artifact; cutover-ready gate qualified on SQLite and live MariaDB / 全required checkをcollector＋versioned DBAccess smoke artifactから組立、cutover-ready gateをSQLite＋live MariaDBで認定 |
| 873 | Explicit cutover and rollback contract / 明示cutover・rollback contract | Freeze writes, take final delta-free snapshot, switch deployment config, smoke test, and retain source for rollback window | `DONE` | side-effect-free cutover contract, operator package, and digest-chained rehearsal package require freeze, final verification, smoke, source retention, rollback window, dry-run rehearsal, rollback rehearsal, safe refs, and explicit approvals / 副作用なしcutover contract・operator package・digest chain付きrehearsal packageでfreeze・final verification・smoke・source保持・rollback window・dry-run rehearsal・rollback rehearsal・安全な参照・明示承認を必須化済み |
| 874 | Representative SQLite-to-MySQL promotion sample / 代表SQLiteからMySQL promotion sample | Prove the whole offline path on one bounded app with relations, JSON/time values, interruption, retry, cutover, and rollback rehearsal | `DONE_SUPPORTED_ARTIFACT_TUTORIAL` | `sample33-sqlite-to-mysql-promotion` provides a reproducible artifact-chain tutorial fixture plus a side-effect-free operator validator CLI for parent/child data, JSON/BLOB/timestamp values, cutover, operator package, and rehearsal package; no runtime wrapper is required until a hands-on operation need appears / `sample33-sqlite-to-mysql-promotion`で親子data・JSON/BLOB/timestamp・cutover・operator package・rehearsal packageを再現するartifact-chain tutorial fixtureと副作用なしoperator validator CLIを追加、hands-on運用需要が出るまでruntime wrapper不要 |
| 875 | SSO app-user promotion qualification / SSO app-user promotion認定 | Migrate the qualified SQLite SSO app-user fixture while preserving IDs, external identity uniqueness, FKs, profile data, and resolver behavior | `DONE_MYSQL_RESOLVER_QUALIFIED` | runtime-created SQLite SSO app-user data enters the promotion chain; the promoted MySQL/MariaDB target restores the same SSO user, preserves app-owned FK data, refreshes safe profile data, and supports a new JIT SSO login through the resolver / runtimeで作成したSQLite SSO app-user dataをpromotion chainへ接続し、昇格後MySQL・MariaDB targetで同一SSO user復元、app-owned FK data保持、safe profile更新、新規JIT SSO loginをresolver経由で実証済み |
| 876 | Promotion lane checkpoint and integration / promotion lane checkpoint・integration | Record supported boundary, tests, operational guide, remaining demand-driven scale work, and PR shape | `DONE_INTEGRATION_READY` | v1 offline promotion has a stable guide, proof/current-workflow entrances, test evidence, explicit non-goals, and a PR-ready semantic stack / v1 offline promotionは恒久guide・proof/current-workflow導線・test evidence・明示non-goals・PR-readyな意味単位stackまで整理済み |
| 877 | Optional scan/Ollama fallback generalization preflight / optional scan・Ollama fallback汎用化事前設計 | Inventory the working Sample19 first slice, fix the generic task-packet boundary, supported inputs/outputs, local-only policy, and non-goals before code changes | `DONE_PREFLIGHT` | reusable validator/task/scan/fallback ownership parts, Sample19 coupling, non-goals, constraints, and #878 deterministic scan first slice recorded / 再利用可能なvalidator・task・scan・fallback ownership、Sample19依存、non-goals、制約、#878 deterministic scan first sliceを記録済み |
| 878 | Generic deterministic scan contract / 汎用deterministic scan contract | Generalize the hash-bound advisory pointer/type scan without adding AI inference or mutation authority | `DONE_GENERIC_SCAN` | provider-free JSON source scan contract added with configurable root pointer, advisory authority, stable source hash binding, no inference, Sample19 compatibility, and focused tests / provider非依存のJSON source scan contractを追加し、root pointer指定・advisory authority・source hash binding・推論なし・Sample19互換・focused testで認定 |
| 879 | Generic optional local-fallback CLI contract / 汎用optional local fallback CLI contract | Replace the Sample19-only command boundary with a task-packet-driven command that still requires an explicit execution flag | `DONE_GENERIC_CLI_CONTRACT` | generic task-bound fallback runner/CLI reads declared hash-bound inputs, refuses implicit execution, writes only declared advisory fallback candidate/validation artifacts, and Sample19 Ollama wrapper delegates to it / 汎用task-bound fallback runner・CLIが宣言済みhash-bound inputを読み、暗黙実行を拒否し、宣言済みadvisory fallback candidate・validation artifactだけを書き、Sample19 Ollama wrapperも委譲済み |
| 880 | Configurable Ollama local adapter / 設定可能Ollama local adapter | Make endpoint, model, timeout, and context explicit validated local configuration instead of hard-coded Sample19 constants | `DONE_CONFIGURABLE_LOCAL_ADAPTER` | local-only Ollama adapter added with validated endpoint/model/timeout/context/temperature, credential-free config, fake-transport tests, and Sample19 wrapper delegation; normal tests do not require Ollama / local-only Ollama adapterを追加し、endpoint・model・timeout・context・temperature検証、credential不要config、fake transport test、Sample19 wrapper委譲を実装。通常testでOllama不要 |
| 881 | Shared validation and advisory artifact integration / 共通validation・advisory artifact統合 | Route fallback candidates through the same authoritative validator and keep them separate from agent-owned output and accepted artifacts | `DONE_PIPELINE_AUTHORITY_METADATA` | task packets and validation results now expose the shared validator plus formal vs advisory candidate authority; fallback remains mutation-free and must be reviewed/copied or adapted into formal output before acceptance / task packetとvalidation resultが共通validatorおよびformal/advisory candidate authorityを明示。fallbackはmutation-freeのままで、accept前にreviewしてformal outputへcopy/adaptする必要あり |
| 882 | AI workspace handoff and operator guide / AI workspace handoff・運用guide | Define where scan/fallback artifacts live and how Codex/Claude asks for confirmation, reviews them, and promotes only a newly validated formal candidate | `DONE_HANDOFF_GUIDE` | stable AI schema proposal handoff guide added with task-root paths, authority order, confirmation wording, formal vs fallback commands, copy/adapt rule, validation metadata, and docs entrances / task root path・authority順・確認文・formal/fallback command・copy/adapt rule・validation metadata・docs導線を持つ恒久handoff guideを追加 |
| 883 | Optional fallback qualification and checkpoint / optional fallback認定・checkpoint | Add fake-transport contract tests, an opt-in real local Ollama smoke, compatibility evidence for Sample19, and a supported-boundary report | `DONE_CHECKPOINT_REAL_SMOKE_DEFERRED` | fake-transport and full tests qualify the supported boundary; local Ollama endpoint/model preflight passed, but the real generation smoke was interrupted after 240s with no artifacts written, so real model E2E remains opt-in/manual rather than a normal gate / fake transport・full testでsupported boundaryを認定。local Ollama endpoint/model preflightは成功したが、実生成smokeは240秒で中断しartifact未作成のため、real model E2Eは通常gateではなくopt-in/manualに据え置き |
| 884 | Promotion and optional fallback stack checkpoint / promotion・optional fallback stack checkpoint | Reconfirm divergence, semantic commit shape, test evidence, supported boundary, and PR readiness after #867-#883 | `DONE_LOCAL_PR_READY` | local branch is clean, 32 commits ahead of `origin/develop`, full test evidence recorded, no push performed without explicit request, and PR can be prepared as feature-to-develop integration / local branchはclean、`origin/develop`より32 commit ahead、full test evidence記録済み、明示依頼なしではpushせず、featureからdevelopへのPR準備可能 |
