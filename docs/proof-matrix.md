...
| `AI-CONTEXT-MD` | `planned` | current plan only / 現在は計画のみ |
| `AUDIT-MD` | `planned` | current plan only / 現在は計画のみ |

## Database Layer Matrix / DB layer matrix

English:
Database support must always name the layer. PostgreSQL user DB output support does not mean PostgreSQL config-store support.

日本語:
DB 対応は必ず layer を指定します。PostgreSQL user DB output 対応は、PostgreSQL config-store 対応を意味しません。
...
English:
PostgreSQL live lanes are opt-in and require explicit DSN / credential setup.

日本語:
PostgreSQL live lane は opt-in で、明示的な DSN / credential 設定が必要です。
...
- AI context or modernization audit generated output / AI context または modernization audit の生成 output。
