...
| `MODERNIZATION-AUDIT-MD` | `current / verified` | `sample17`, `Sample17MultiOutputProjectTest`, generated read-only diagnostic output |

## Database Layer Matrix / DB layer matrix

English:
Database support must always name the layer. PostgreSQL completion is defined for user DB / generated output / input import. PostgreSQL config-store support is intentionally not required.

日本語:
DB 対応は必ず layer を指定します。PostgreSQL 完了は user DB・generated output・input import 側で定義します。PostgreSQL config-store 対応は意図的に不要です。
