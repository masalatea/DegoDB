...
English companion:
This summary explains the responsibility of each top-level area in the repo. Read it when you need to decide where durable source of truth belongs, where disposable output should go, and why `original-codes/` must remain host-side reference only.
...
| `original-codes/` | host-side reference only | 旧実装の調査、差分確認、host-side export input | current runtime input、Docker mount 前提の mainline |
