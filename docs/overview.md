...
no-code UI の検証は、軽い contract test を内側に置き、browser smoke を外側の代表確認として扱う。まず `screen-definition.json` / `runtime-preview.json` / `runtime-preview.html` を PHPUnit と `DOMDocument` で検査し、必要になった時だけ lightweight DOM runtime を追加する。headless Chrome / Playwright は public preview、current / alias、auth、submit handoff のような browser integration gate に限定する。詳細は [No-Code UI Testing](no-code-ui-testing.md) を参照する。
