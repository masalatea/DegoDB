...
sample18-failure-runtime-reference-fixture: ## Sample18 SQL後失敗の一時runtime referenceを生成する
	@test -n "$(OUTPUT_DIR)" || (echo "OUTPUT_DIR is required" >&2; exit 1)
	php mtool/scripts/create_sample18_failure_runtime_reference.php \
		sample/tutorials/sample18-mini-task-board-demo/reference \
		$(OUTPUT_DIR)

sample18-guarded-transaction-http-smoke: ## Sample18 guarded routeの実commit・rollbackを隔離stackで検証する
	bash mtool/scripts/check_sample18_guarded_transaction_http_smoke.sh
